...
// BatchCalculatePreferredReminderHour calculates the preferred reminder hour
// for multiple users at once. Returns a map of userID -> preferred hour.
func (r *Repo) BatchCalculatePreferredReminderHour(ctx context.Context, userIDs []int64) (map[int64]int, error) {
	result := make(map[int64]int)
	for _, userID := range userIDs {
		result[userID] = 9 // Default
	}
	...
	for i := 0; i < len(userIDs); i += chunkSize {
		...
		query := `
			...
			FROM weight_logs
			WHERE measured_at >= ? AND user_id IN (`

		args := make([]interface{}, 1, len(chunk)+1)
		args[0] = since
		...
		measurements := make(map[int64][]time.Time)
		for rows.Next() {
			var userID int64
			var measuredAt time.Time
			...
			measurements[userID] = append(measurements[userID], measuredAt)
		}
		rows.Close()

		for userID, times := range measurements {
			if len(times) >= 3 {
				totalHour := 0
				for _, t := range times {
					totalHour += t.Hour()
				}
				avgHour := totalHour / len(times)
				if avgHour < 6 {
					avgHour = 6
				} else if avgHour > 12 {
					avgHour = 12
