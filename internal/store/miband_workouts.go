...
// Returns inserted=true if a new row was created, inserted=false if an existing row was updated.
func (s *Store) InsertMiBandWorkout(ctx context.Context, w *MiBandWorkout) (bool, error) {
	...
	// Check if the workout already exists to distinguish insert from update.
...
// Returns (imported, skipped, error). imported includes both new inserts and updates.
func (s *Store) ImportMiBandWorkouts(ctx context.Context, workouts []MiBandWorkout, gpsTracks map[int64][]MiBandGPSPoint) (int, int, error) {
	...
	for i := range workouts {
		...
		// Check if the workout already exists to skip GPS re-insertion on update.
		var existingID int64
		err := s.db.QueryRowContext(ctx, `
			SELECT id FROM miband_workouts WHERE user_id = ? AND source_start_ms = ?`,
			w.UserID, w.SourceStartMs).Scan(&existingID)
		...
		// Only insert GPS track points for new workouts (GPS data is immutable from device)
		if isNew {
