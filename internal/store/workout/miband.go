...
func (r *Repo) ImportMiBandWorkouts(ctx context.Context, workouts []MiBandWorkout, gpsTracks map[int64][]MiBandGPSPoint) (int, int, error) {
	...
	for i := range workouts {
		...
		isNew, err := r.importSingleWorkout(ctx, w, gpsTracks)
		if err != nil {
...
func (r *Repo) importSingleWorkout(ctx context.Context, w *MiBandWorkout, gpsTracks map[int64][]MiBandGPSPoint) (bool, error) {
	...
	if isNew {
		if err := insertGPSBatched(ctx, conn, workoutID, gpsTracks[w.SourceStartMs]); err != nil {
			return false, err
		}
	}
...
func insertGPSBatched(ctx context.Context, conn *sql.Conn, workoutID int64, pts []MiBandGPSPoint) error {
	if len(pts) == 0 {
		return nil
	}
