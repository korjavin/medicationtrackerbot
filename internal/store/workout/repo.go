// Package workout owns the workout_groups, workout_variants,
// workout_exercises, workout_sessions, workout_exercise_logs,
...
// Repo is the workout repository. Construct with New; share one *Repo per
...
func (r *Repo) DeleteGroup(id int64) error {
	...
	if _, err := r.db.Exec("DELETE FROM workout_rotation_state WHERE group_id = ?", id); err != nil {
		return err
	}
	// Delete schedule snapshots
	if _, err := r.db.Exec("DELETE FROM workout_schedule_snapshots WHERE group_id = ?", id); err != nil {
		return err
	}
	// Delete variants (exercises already verified empty)
	if _, err := r.db.Exec("DELETE FROM workout_variants WHERE group_id = ?", id); err != nil {
		return err
	}
	// Delete the group
	_, err = r.db.Exec("DELETE FROM workout_groups WHERE id = ?", id)
	return err
}
...
func (r *Repo) DeleteVariant(id int64) error {
	...
	_, err = r.db.Exec("DELETE FROM workout_variants WHERE id = ?", id)
	return err
}
...
func (r *Repo) ListExerciseLibrary(userID int64) ([]ExerciseLibraryItem, error) {
	rows, err := r.db.Query(`
...
func (r *Repo) GetExerciseLibraryItem(id int64) (*ExerciseLibraryItem, error) {
	var item ExerciseLibraryItem
	var sets, repsMin sql.NullInt64
	var repsMax sql.NullInt64
	var weightKg sql.NullFloat64
...
func (r *Repo) AdvanceRotation(groupID int64) error {
	// Get current state
	state, err := r.GetRotationState(groupID)
	if err != nil {
		return err
	}
	if state == nil {
	...
	variants, err := r.ListVariantsByGroup(groupID)
	if err != nil {
		return err
	}
...
func (r *Repo) ListPendingAdHocSessions(userID int64, before time.Time) ([]WorkoutSession, error) {
...
func (r *Repo) IsAdHocSession(sessionID int64) (bool, error) {
	var groupID int64
	err := r.db.QueryRow(`SELECT group_id FROM workout_sessions WHERE id = ?`, sessionID).Scan(&groupID)
	if err == sql.ErrNoRows {
...
func (r *Repo) GetLatestSessionScheduledDate(groupID, userID int64) (time.Time, bool, error) {
...
func (r *Repo) UpdateSessionStatus(id int64, status string) error {
	_, err := r.db.Exec("UPDATE workout_sessions SET status = ? WHERE id = ?", status, id)
	return err
}
...
func (r *Repo) StartSession(id int64) error {
	_, err := r.db.Exec(`
...
func (r *Repo) UpdateSessionVariant(id int64, variantID int64) error {
	_, err := r.db.Exec(`
		UPDATE workout_sessions
		SET variant_id = ?
...
func (r *Repo) CompleteSession(id int64) error {
	_, err := r.db.Exec(`
		UPDATE workout_sessions
...
func (r *Repo) SkipSession(id int64) error {
	_, err := r.db.Exec("UPDATE workout_sessions SET status = 'skipped' WHERE id = ?", id)
	return err
}

func (r *Repo) PreSkipSession(id int64) error {
	_, err := r.db.Exec("UPDATE workout_sessions SET status = 'pre_skipped' WHERE id = ?", id)
	return err
}

func (r *Repo) CancelPreSkip(id int64) error {
	_, err := r.db.Exec("UPDATE workout_sessions SET status = 'pending' WHERE id = ?", id)
	return err
}
...
func (r *Repo) DeleteSession(id int64) error {
	...
	// session deletion.
	if _, err := r.db.Exec("DELETE FROM workout_exercise_logs WHERE session_id = ?", id); err != nil {
		return err
	}
	_, err := r.db.Exec("DELETE FROM workout_sessions WHERE id = ?", id)
	return err
}

func (r *Repo) SnoozeSession(id int64, snoozeDuration time.Duration) error {
...
func (r *Repo) ClearSnooze(id int64) error {
	_, err := r.db.Exec(`
		UPDATE workout_sessions
...
func (r *Repo) SetSessionNotificationMessageID(id int64, messageID int) error {
	_, err := r.db.Exec("UPDATE workout_sessions SET notification_message_id = ? WHERE id = ?", messageID, id)
	return err
}

// -- Exercise Log Methods --

func (r *Repo) LogExercise(sessionID, exerciseID int64, exerciseName string, setsCompleted, repsCompleted *int, weightKg *float64, status, notes string) (int64, error) {
	return r.LogExerciseWithSource(sessionID, exerciseID, exerciseName, setsCompleted, repsCompleted, weightKg, status, notes, "schedule")
}
...
func (r *Repo) LogExerciseWithSource(sessionID, exerciseID int64, exerciseName string, setsCompleted, repsCompleted *int, weightKg *float64, status, notes, source string) (int64, error) {
	res, err := r.db.Exec(`
...
func (r *Repo) DeleteExerciseLog(id int64) error {
	_, err := r.db.Exec("DELETE FROM workout_exercise_logs WHERE id = ?", id)
	return err
}
...
func (r *Repo) UpsertExerciseLogByName(ctx context.Context, sessionID, exerciseID int64, exerciseName string, setsCompleted, repsCompleted *int, weightKg *float64, status, notes, source string, loggedAt time.Time) (int64, bool, error) {
...
func (r *Repo) SetExerciseLogSource(id int64, source string) error {
	_, err := r.db.Exec("UPDATE workout_exercise_logs SET source = ? WHERE id = ?", source, id)
	return err
}

// GetExerciseLogByID fetches a single exercise log by its primary key.
func (r *Repo) GetExerciseLogByID(id int64) (*WorkoutExerciseLog, error) {
	...
	if repsCompleted.Valid {
		rr := int(repsCompleted.Int64)
		log.RepsCompleted = &rr
	}
	if weightKg.Valid {
...
func (r *Repo) GetExerciseLogBySessionExerciseSource(sessionID, exerciseID int64, source string) (*WorkoutExerciseLog, error) {
	var log WorkoutExerciseLog
	var setsCompleted, repsCompleted sql.NullInt64
	var weightKg sql.NullFloat64
	var notes sql.NullString
...
func (r *Repo) PropagateExerciseToSchedule(sessionID int64, exerciseID int64, exerciseName string, sets *int, reps *int, weight *float64) error {
	_, err := r.db.Exec(`
		UPDATE workout_exercises
...
func (r *Repo) GetExerciseLogBySessionAndExercise(sessionID, exerciseID int64) (*WorkoutExerciseLog, error) {
	var log WorkoutExerciseLog
	var setsCompleted, repsCompleted sql.NullInt64
	var weightKg sql.NullFloat64
	var notes sql.NullString
...
func (r *Repo) CreateGroupSnapshot(groupID int64, snapshotData, changeReason string) error {
	_, err := r.db.Exec(`
...
func (r *Repo) ListRecentExerciseLogsByName(ctx context.Context, userID int64, exerciseName string, limit int) ([]WorkoutExerciseLog, error) {
	if limit <= 0 {
		limit = 1
	}
...
// scanSession scans a row from workout_sessions into a WorkoutSession. The
...
// scanExerciseLog scans a row from workout_exercise_logs into a WorkoutExerciseLog.
func scanExerciseLog(rows *sql.Rows) (WorkoutExerciseLog, error) {
	var log WorkoutExerciseLog
	var setsCompleted, repsCompleted sql.NullInt64
	var weightKg sql.NullFloat64
	var notes sql.NullString
