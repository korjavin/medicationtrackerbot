...
// Task 7's dry-queue sweep cadence and thresholds. The warning fires at most
// once a day per account (warnCooldown) once the account's scheduled-push queue
// is close to running dry — or has already run dry — within
// dryQueueWarnWithin (default 120h). Sync recency is deliberately not part of
// the predicate; see AccountsNeedingStaleSyncWarning (bd med-2lx).
const (
...
// StaleSyncSweep sends the generic dry-queue warning (Task 7) to every account
// whose scheduled-push queue is about to run dry — or already has — at most
// once per warnCooldown per account. Exported so tests can drive it without
