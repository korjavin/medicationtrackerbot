...
const usageProtocol = "Decision rule: (1) Discover — call mcp_help with no args (or topic=/query=) to scan the catalog, then drill in with operation_id=/operation_ids=[...] for full schemas + a runnable example. (2) Run one operation — mcp_call(operation_id, params, path_params, body). (3) Run multiple steps (loops, joins, or deriving a value from many rows) — write an mcp_execute Python script. An aggregate (average, sum, count, min/max, grouping) or a join across operations belongs in step (3): compute it inside the script and output() it, so the result is exact. " +
	"Rules: an mcp_execute script calls output(value) exactly once (zero or multiple calls abort the run). Params are passed as a query-string object (params={...}); placeholders like {id} in an operation's route go in path_params={...}, not params. Writes require mode='write' and a one-sentence intent. Timestamps use the user's stored timezone unless an operation accepts an explicit tz/tz_offset; for relative dates ('today', 'now', 'yesterday', 'last N days') use this response's current_time as the real clock rather than guessing the date or year. " +
	"Resolving references: positional or recency words ('first', 'next', 'last', 'latest', 'most recent', 'Nth', 'top') refer to the order the matching list operation returns — read the list, then act on that exact row. " +
	"Safety: there is no bulk-delete or 'wipe everything' operation — destructive ops delete one record by id. Before a destructive write (delete/archive/overwrite), and before anything that would touch many records, ask the user to confirm, stating exactly what will change; don't chain single deletes to fake a bulk wipe. If a request can't be met with the available operations or is ambiguous (which item? what change?), ask one clarifying question. Finish each turn with a short plain-text reply (the answer, the confirmation question, or the limitation)."
...
func (s *Server) buildHelp(ctx context.Context, req *sdkmcp.CallToolRequest, input HelpInput) (*sdkmcp.CallToolResult, HelpResponse, error) {
	...
	if query != "" {
		...
		return nil, HelpResponse{
			...
			Count:             len(ops),
			Note:              fmt.Sprintf("Showing %d match(es) for query %q. These are operations, not data; re-running the search returns the same list.", len(ops), query),
			NextStep:          fmt.Sprintf("Run one with mcp_call(operation_id=%q, …) — or whichever id above fits the request — for a single read/write (writes need mode=\"write\" + a one-sentence intent), or mcp_execute for multi-step math. If the answer needs the newest/Nth item, list first, then act on that exact row. For a field's exact type, mcp_help(operation_id=…) returns the full schema.", ops[0].ID),
			NextTools:         []string{"mcp_call", "mcp_execute"},
		}, nil
	}
