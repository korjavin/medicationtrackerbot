...
output(result)`,
  ...
  "properties": {
    "range": {"type": "string", "enum": ["7d", "30d", "90d", "all"], "description": "Window the counts and top_exercises cover. Default 30d."}
			...
			Description:     "Get aggregated workout statistics: session counts, completion rate, the current weekly streak, the most-trained exercises, per-week and per-day activity breakdowns, and the training-load aggregates (working volume / hard sets / reps / PRs, the named weight records behind that PR count, per-week tonnage, and per-exercise totals for EVERY exercise trained in the window). `range` scopes the counts, totals, prs, daily_activity, top_exercises and exercise_totals (default 30d); current_streak_weeks and longest_streak_weeks are always whole-history. Warm-up sets (set_type \"warmup\") are excluded from totals, weekly_volume, top_exercises and exercise_totals — they are not working volume, so an exercise logged with warm-ups ONLY has no working sets and is absent from exercise_totals and top_exercises entirely. A HARD set is a working set taken near failure: RIR <= 4, i.e. the logged per-set rpe >= 6. Per-set effort is optional, so a working set with no rpe still counts as hard (\"no opinion\", never \"too easy\"); the rated-but-easy sets that were excluded are reported separately as totals.easy_sets. Effort and coverage are separate fields, so read the one you mean: exercise_totals[].sets counts EVERY working set (ungated — this is the one that answers \"did I train this at all\"), while exercise_totals[].hard_sets, totals.hard_sets and weekly_volume[].hard_sets are effort-gated. There is no per-group breakdown — filter workouts.sessions.list on group_id for that.",
			ResponseSummary: "Stats object with range (the echoed window), total_sessions, completed_sessions, skipped_sessions, completion_rate (percent, 0-100), active_weeks, current_streak_weeks, longest_streak_weeks (the longest run of consecutive trained weeks anywhere in the history, >= current_streak_weeks by construction), top_exercises[], weekly_activity[], daily_activity[], totals{volume_kg,hard_sets,easy_sets,reps,pr_count}, prs[{exercise_name,date,weight_kg,previous_kg}], weekly_volume[] and exercise_totals[{exercise_name,session_count,sets,hard_sets,reps,total_volume_kg,max_weight_kg}]. weekly_activity buckets by ISO Monday; daily_activity is one entry per LOCAL calendar day inside `range` that saw a completed or skipped session (sparse — rest days are simply absent), ascending by date. top_exercises is the top-8 slice of exercise_totals (same volume math, so the two always agree); exercise_totals covers every exercise with at least one working set in the window. prs names the records behind totals.pr_count — one entry per in-range log whose heaviest working set beat every earlier log of that exercise, newest first, where date is the session's LOCAL calendar day and previous_kg is the record it broke (0 for a first-ever weighted lift). Weight PRs only; est-1RM and rep records are not derived here. top_exercises/weekly_activity/daily_activity/prs/weekly_volume/exercise_totals are null (not []) when the window holds nothing.",
			ResponseExample: `{
  "range": "30d",
  "total_sessions": 48, "completed_sessions": 40, "skipped_sessions": 8, "completion_rate": 83.3, "active_weeks": 12, "current_streak_weeks": 4, "longest_streak_weeks": 9,
  "top_exercises": [
  ...
  "prs": [
    {"exercise_name": "Squat", "date": "2026-05-01", "weight_kg": 100.0, "previous_kg": 92.5}
  ],
  "weekly_volume": [
