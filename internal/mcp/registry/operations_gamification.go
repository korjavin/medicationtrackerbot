...
output(result)`,
		},
		{
			...
			ResponseSummary: "Object {enabled, targets[] of {metric_key, low, high, falloff, recommended_low, recommended_high, recommended_falloff, is_custom, is_recommended}}. Metrics: bp_systolic, bp_diastolic, resting_hr, sleep_hours, steps, bedtime (minutes of deviation from the user's trailing bedtime median).",
			ResponseExample: `{
  "enabled": true,
  "targets": [
...
# Tier 3 unlocked, tier 4 not: {"enabled": true, "sleep_bp": {...}, "good_day": {"locked": true, "unlocks_at_level": 7}}`,
		},
		{
			...
			Risk:            RiskRead,
			Description:     "Gauge-trend read model: weight as smoothed velocity/acceleration vs the user's goal pace, BP as a rolling in-range share vs a 60-day baseline, and resting HR as a 14-day mean vs baseline. Daily body-metric noise is smoothed out by design — a single bad day barely moves any of these. Each gauge reports insufficient_data instead of a distorted number below its minimum sample count.",
