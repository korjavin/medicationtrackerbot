...
func TestMCPWorkoutLog_PartialSuccess(t *testing.T) {
	...
	if _, err := db.Workout.LogExerciseWithSource(sess.ID, 0, "Inclined Press", intPtr(3), intPtr(8), floatPtr(60), "completed", "", "library"); err != nil {
		t.Fatalf("seed inclined press: %v", err)
	}
	if _, err := db.Workout.LogExerciseWithSource(sess.ID, 0, "Biceps Curls", intPtr(3), intPtr(10), floatPtr(12.5), "completed", "", "library"); err != nil {
...
func TestMCPWorkoutLog_DeleteRejectsForeignSession(t *testing.T) {
	...
	if _, err := db.Workout.LogExerciseWithSource(foreign.ID, 0, "Squat", intPtr(3), intPtr(8), floatPtr(80), "completed", "", "library"); err != nil {
		t.Fatalf("seed: %v", err)
	}
