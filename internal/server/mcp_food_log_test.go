...
func TestHandleMCPFoodLog_ValidRequest(t *testing.T) {
	...
	logs, err := db.Food.ListLogs(context.Background(), 123456, time.Date(2026, 3, 1, 0, 0, 0, 0, time.UTC), 1)
	if err != nil {
	...
	prod, err := db.Food.GetProductByName(context.Background(), 123456, "Pasta")
	if err != nil {
