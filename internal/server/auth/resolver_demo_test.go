...
func TestDemoUserResolver_AlwaysReturnsConfiguredUser(t *testing.T) {
	resolver := NewDemoUserResolver(User{ID: 1, FirstName: "Demo", Username: "demo"})
	...
	if got.FirstName != "Demo" {
		t.Errorf("FirstName = %q, want Demo", got.FirstName)
	}
}

func TestDemoUserResolver_IgnoresRequestHeadersAndCookies(t *testing.T) {
	resolver := NewDemoUserResolver(User{ID: 42, FirstName: "Demo"})

	// Even with a session cookie and initData header, the demo resolver
...
func TestDemoUserResolver_ReturnsCopy(t *testing.T) {
	original := User{ID: 7, FirstName: "Original"}
	resolver := NewDemoUserResolver(original)
