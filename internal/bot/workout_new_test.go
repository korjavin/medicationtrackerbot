...
func TestWorkoutStart_AdHocSendsConfirmation(t *testing.T) {
	...
	session, err := env.s.Workout.CreatePlannedAdHocSession(userID, scheduled, "12:00")
	if err != nil {
		t.Fatalf("CreatePlannedAdHocSession: %v", err)
	}
	if _, err := env.s.Workout.LogExerciseWithSource(session.ID, 0, "Burpees", nil, nil, nil, "", "", "schedule"); err != nil {
...
func TestWorkoutSkip_AdvancesRotationForRotatingGroup(t *testing.T) {
	...
	rotState, err := env.s.Workout.GetRotationState(group.ID)
	if err != nil {
...
func TestExerciseDone_ExistingSkippedLogBecomesCompleted(t *testing.T) {
	...
	if _, err := env.s.Workout.LogExercise(sessionID, exerciseID, "Pull-up", nil, nil, nil, "skipped", ""); err != nil {
	...
	log, err := env.s.Workout.GetExerciseLogBySessionAndExercise(sessionID, exerciseID)
	if err != nil {
...
func TestExerciseDone_VariantMismatch_FallsThruToLibrary(t *testing.T) {
	...
	groupA, err := env.s.Workout.CreateGroup("Push", "", false, userID, "[1]", "09:00", 15)
	if err != nil {
		t.Fatalf("CreateWorkoutGroup A: %v", err)
	}
	variantA, err := env.s.Workout.CreateVariant(groupA.ID, "Heavy", nil, "")
	if err != nil {
	...
	session, err := env.s.Workout.CreateSession(groupB.ID, variantB.ID, userID, time.Now(), "09:00")
	if err != nil {
	...
	logs, err := env.s.Workout.ListExerciseLogs(session.ID)
	if err != nil {
...
func TestExerciseDone_SameVariant_UsesWorkoutExercise(t *testing.T) {
	...
	group, err := env.s.Workout.CreateGroup("Push", "", false, userID, "[1]", "09:00", 15)
	if err != nil {
