...
func (b *Bot) handleBPReminderCallback(cb *tgbotapi.CallbackQuery, data string) {
	...
	case "bp_snooze":
		// Snooze for 2 hours
		if err := b.reminderSvc.SnoozeBPReminder(context.Background(), cb.From.ID); err != nil {
	...
	case "bp_dontbug":
		// Block for 24 hours
		if err := b.reminderSvc.BlockBPReminders(context.Background(), cb.From.ID); err != nil {
