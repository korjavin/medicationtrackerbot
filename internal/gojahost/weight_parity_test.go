...
// weightUserID is the single account the Go side seeds under.
const weightUserID int64 = 1

// weightFixture is one measurement, feeding BOTH the JS domain (weight.create)
...
// jsCreateWeight drives one JS create through the host and returns its response.
func jsCreateWeight(t *testing.T, h *Host, f weightFixture) map[string]interface{} {
	t.Helper()
	b, err := json.Marshal(f.jsInput())
	if err != nil {
		t.Fatalf("marshal js input: %v", err)
	}
	v, err := h.Call("weight", "create", string(b))
	if err != nil {
		t.Fatalf("js weight.create(%s): %v", b, err)
	}
	resp, ok := v.Export().(map[string]interface{})
	if !ok {
		t.Fatalf("js weight.create(%s) result not an object: %T", b, v.Export())
	}
	return resp
}

// goCreateWeight replicates handleCreateWeight's trend seeding: read the last
// log's trend, EWMA it forward via CalculateWeightTrend (alpha=0.1), CreateLog.
...
// asFloat normalizes a goja-exported number to float64.
...
// "previous = latest by measured_at" selection is identical on both sides.
...
// weight.create → weight_trend in the response, through a pooled VM) must equal
// the native weight.CalculateWeightTrend across the whole EWMA sequence,
// bit-for-bit (identical IEEE-754 op order on both sides).
...
// (create then list) and the Go store (handler-equivalent create then ListLogs)
// must yield field-by-field-equal logs in the same DESC-by-measured_at order.
func TestWeightCreateListParity(t *testing.T) {
	...
	v, err := h.Call("weight", "list", `{ days: 30, limit: 100 }`)
	if err != nil {
		t.Fatalf("js weight.list: %v", err)
	}
