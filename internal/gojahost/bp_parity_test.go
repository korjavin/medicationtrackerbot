...
// bpUserID is the single account the Go side seeds under.
...
// bpFixture is one reading, seeding BOTH the JS domain (via a JSON object fed to
// domain.create through the pooled host) and the Go store (via CreateReading)
// from a single source of truth. The two sides run independently and their
// outputs are compared field-by-field — this is what proves value-exact LOGIC
// parity (category resolution, ordering, stats) between web/domain/bp.js and
// internal/store/bp, which the generic records port alone could not.
...
// jsCreate drives one JS create through the host and asserts it fulfilled.
func jsCreate(t *testing.T, h *Host, f bpFixture) {
	t.Helper()
	b, err := json.Marshal(f.jsInput())
	if err != nil {
		t.Fatalf("marshal js input: %v", err)
	}
	if _, err := h.Call("bp", "create", string(b)); err != nil {
		t.Fatalf("js bp.create(%s): %v", b, err)
	}
}
...
// strOf returns js[key] as a string, or "" when absent.
...
// bp.create → resolveCategory in a pooled VM) must return the SAME bucket as the
// native bp.CalculateBPCategory for every fixture, across all buckets.
func TestBPCategoryParity(t *testing.T) {
	h := newTestHost(t)

	fixtures := []struct{ sys, dia int }{
		{110, 70}, {119, 79}, {120, 70}, {125, 75}, {129, 79},
		{130, 79}, {120, 80}, {135, 85}, {140, 85}, {130, 90},
		{150, 100}, {180, 100}, {150, 120}, {181, 100}, {150, 121}, {200, 130},
	}

	for _, f := range fixtures {
		want := bp.CalculateBPCategory(f.sys, f.dia)
		v, err := h.Call("bp", "create", fmt.Sprintf(
			`{ measured_at: '2026-05-20T10:00:00Z', systolic: %d, diastolic: %d }`, f.sys, f.dia))
		if err != nil {
			t.Fatalf("js bp.create(%d/%d): %v", f.sys, f.dia, err)
		}
		resp, ok := v.Export().(map[string]interface{})
		if !ok {
			t.Fatalf("js bp.create(%d/%d) result not an object: %T", f.sys, f.dia, v.Export())
		}
		got, _ := resp["category"].(string)
		if got != want {
...
func TestBPCreateListParity(t *testing.T) {
	...
	v, err := h.Call("bp", "list", `{ days: 30, limit: 100 }`)
	if err != nil {
		t.Fatalf("js bp.list: %v", err)
	}
	...
	for i := range goReadings {
		...
		for _, tc := range []struct {
			...
			if tc.js != tc.gov {
				t.Errorf("reading[%d] %s: JS %q vs Go %q", i, tc.name, tc.js, tc.gov)
			}
		}
...
// aggregates for every period (14/30/60), exercising the Intl-backed
...
func TestBPStatsParity(t *testing.T) {
	...
	fixtures := []bpFixture{
		{measuredAt: "2026-05-05T16:00:00Z", sys: 100, dia: 62},
		{measuredAt: "2026-05-25T16:00:00Z", sys: 140, dia: 90},
		{measuredAt: "2026-06-15T12:00:00Z", sys: 120, dia: 80},
		...
		{measuredAt: "2026-06-19T15:00:00Z", sys: 200, dia: 120, ignoreCalc: true},
	}
	for _, f := range fixtures {
		jsCreate(t, h, f)
		goCreate(t, r, ctx, f)
	...
	v, err := h.Call("bp", "getStats", "")
	if err != nil {
		t.Fatalf("js bp.getStats: %v", err)
	}
...
// cmpPeriod asserts a JS period object equals the Go BPPeriodStats.
