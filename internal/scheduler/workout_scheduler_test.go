...
func TestWorkoutCheckerScenarios(t *testing.T) {
	...
	testharness.RunScenarios(t, filename, func(t *testing.T, s testharness.Scenario) {
		...
		// Our mock notifier just appends to its internal slice, but we updated it to
		// replace the Text with "DELETED" when `Delete()` is called.
		activeNotifications := 0
		for _, n := range mockNotifier.Notifications {
			if n.Text != "DELETED" {
				activeNotifications++
			}
		}
