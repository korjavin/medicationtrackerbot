// Wandergeek stale-data badge — small chip surfaced in section headers
// to communicate read freshness when data was loaded through cachedFetch
// (Task 4 of the local-first read-resilience plan).
//
// Tone selection is deterministic from {fetchedAt, isOffline, staleAfterMs}:
//   • online + recent  → neutral tone (`Updated 5m ago`)
//   • online + old     → neutral tone (`Updated 2h ago`)
//   • offline + fresh  → warning tone, prefixed (`Offline · 12m old`)
//   • offline + stale  → warning tone, prefixed (`Offline · 3h old`)
//   • no fetchedAt     → muted neutral fallback (`Never updated` or
//                         `Offline · no cache`)
//
// API:
...
//                  Defaults to 1h. Online-fresh data (age <= staleAfterMs)
//                  always renders neutral; offline always renders warning
//                  past the threshold.
