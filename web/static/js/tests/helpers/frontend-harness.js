...
const WORKOUT_SHARE_JS = path.join(REPO_ROOT, 'web/static/js/features/workout/share.js');
...
const WORKOUT_EQUIPMENT_JS = path.join(REPO_ROOT, 'web/static/js/features/workout/equipment.js');
...
export function loadFrontendEnv({ withWorkout = false, telegramInitData = '', telegramVersion = '6.9', url = 'https://example.test/' } = {}) {
  ...
  if (withWorkout) {
    ...
    evalFileCached(window, WORKOUT_SHARE_JS);
    ...
    evalFileCached(window, WORKOUT_EQUIPMENT_JS);
