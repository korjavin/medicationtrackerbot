...
describe('INLINE_HANDLER_RE coverage', () => {
    ...
    it('matches every CSP-blocked attribute form', () => {
        expect(matches('<button onclick="foo()">')).toBe(true);
        expect(matches("<button onclick='foo()'>")).toBe(true);
        expect(matches('<button onclick=foo()>')).toBe(true);
        expect(matches('<button onclick="">')).toBe(true);
        expect(matches('<button\n  onclick="foo()">')).toBe(true);
        expect(matches('<button\n  onclick=foo()>')).toBe(true);
        expect(matches('<button\n  ONCLICK="foo()">')).toBe(true);
        // onchange/onsubmit/etc. — the regex is `on[a-z]+`, not just onclick
        expect(matches('<input onchange="x()">')).toBe(true);
        expect(matches('<form onsubmit="x()">')).toBe(true);
    });

    it('matches even when a prior attribute value contains > or <', () => {
        // Regression for codex finding: `[^<>]*?` stopped at the `>`
        // inside `title="1 > 0"`, letting the inline handler slip past
        // the guard.
        expect(matches('<button title="1 > 0" onclick="foo()">')).toBe(true);
        expect(matches("<button title='1 > 0' onclick='foo()'>")).toBe(true);
        expect(matches('<a title="<x>" onclick="foo()">')).toBe(true);
    });

    it('matches when a template-literal interpolation contains > or <', () => {
        ...
        expect(matches('`<button ${count > 0 ? "data-x=1" : ""} onclick="foo()">`')).toBe(true);
        expect(matches('`<button ${count < 10 ? "data-x=1" : ""} onclick="foo()">`')).toBe(true);
        // Nested braces inside the interpolation (object literal) — the
        // mask handles balanced braces so this is still detected.
        expect(matches('`<button ${getProps({foo: 1 > 0})} onclick="x()">`')).toBe(true);
    });

    it('matches when an interpolation contains a string with a } in it', () => {
        ...
        expect(matches('`<button ${foo("}") && count > 0 ? "data-x=1" : ""} onclick="x()">`')).toBe(true);
        expect(matches("`<button ${foo('}') ? 'a' : 'b'} onclick='x()'>`")).toBe(true);
        // `}` inside a single-line comment inside the interpolation.
        expect(matches('`<button ${foo() /* } */ && bar} onclick="x()">`')).toBe(true);
        // `}` inside a nested template literal inside the interpolation.
        expect(matches('`<button ${`a}b`} onclick="x()">`')).toBe(true);
    });

    it('matches when an interpolation contains a regex literal with } or >', () => {
        ...
        expect(matches('`<button ${/}/.test(x) && count > 0 ? "x" : ""} onclick="foo()">`')).toBe(true);
        expect(matches('`<button ${/[}]/.test(x) && count > 0 ? "x" : ""} onclick="foo()">`')).toBe(true);
        // Regex after `&&` is still in regex context.
        expect(matches('`<button ${flag && /a>b/.test(x)} onclick="x()">`')).toBe(true);
        // Char class with escaped `/` and bracket content.
        expect(matches('`<button ${/[a\\/}]/g.test(x) ? "" : ""} onclick="x()">`')).toBe(true);
    });

    it('matches when an interpolation starts with a unary keyword + regex', () => {
        ...
        expect(matches('`<button ${typeof /}/.test(x) && count > 0 ? "x" : ""} onclick="foo()">`')).toBe(true);
        expect(matches('`<button ${void /}/.test(x) && count > 0 ? "x" : ""} onclick="x()">`')).toBe(true);
        expect(matches('`<button ${delete /}/.test(x) && count > 0 ? "x" : ""} onclick="x()">`')).toBe(true);
        expect(matches('`<button ${return /}/.test(x)} onclick="x()">`')).toBe(true);
        expect(matches('`<button ${throw /}/} onclick="x()">`')).toBe(true);
        expect(matches('`<button ${yield /}/} onclick="x()">`')).toBe(true);
        expect(matches('`<button ${await /}/} onclick="x()">`')).toBe(true);
    });

    it('does not over-mask a plain "${" inside a regular string literal', () => {
        ...
        expect(matches('const x = "${"; const html = "<button onclick=foo()>";')).toBe(true);
        // `${` inside a single-quoted string then a real violation.
        expect(matches("const x = '${'; const y = `<button onclick='x()'>`;")).toBe(true);
        // `${` inside a line comment then a real violation.
        expect(matches('// ${ never closed\nconst y = `<button onclick="x()">`;')).toBe(true);
    });

    it('matches when a top-level regex literal contains a backtick', () => {
        ...
        expect(matches('const re = /`/; const html = `<button ${count > 0 ? "x" : ""} onclick="foo()">`;')).toBe(true);
        // Regex with `${` inside also shouldn't fool the masker.
        expect(matches('const re = /\\${/; const html = `<button ${count > 0 ? "x" : ""} onclick="foo()">`;')).toBe(true);
        // Regex literal at start of file (no preceding char).
        expect(matches('/`/.test(""); const html = `<button onclick="x()">`;')).toBe(true);
    });

    it('matches when a post-increment is followed by division', () => {
        ...
        expect(matches('`<button ${a++ / b} onclick="foo()">`')).toBe(true);
        expect(matches('`<button ${a-- / b} onclick="foo()">`')).toBe(true);
        // Prefix increment is also a value; following `/` is still division.
        expect(matches('`<button ${++a / b} onclick="foo()">`')).toBe(true);
    });

    it('matches when a control-flow header is followed by a regex', () => {
        ...
        expect(matches('`<button ${(() => { if (x) /}/.test(y); })() && count > 0 ? "x" : ""} onclick="foo()">`')).toBe(true);
        expect(matches('`<button ${(() => { while (x) /}/.test(y); })() && count > 0 ? "x" : ""} onclick="x()">`')).toBe(true);
        expect(matches('`<button ${(() => { for (;;) /}/.test(y); })() && count > 0 ? "x" : ""} onclick="x()">`')).toBe(true);
        // Regular function-call `)` is still value-context (so `f() / 2`
        // reads as division and we don't accidentally swallow a `}`).
        expect(matches('`<button ${f() / 2 > 0 ? "x" : ""} onclick="foo()">`')).toBe(true);
    });

    it('matches when a top-level control-flow header is followed by a regex with a backtick', () => {
        ...
        expect(matches('if (x) /`/.test(y); const html = `<button ${count > 0 ? "x" : ""} onclick="foo()">`;')).toBe(true);
        expect(matches('while (x) /`/.test(y); const html = `<button ${count > 0 ? "x" : ""} onclick="foo()">`;')).toBe(true);
        expect(matches('for (;;) /`/.test(y); const html = `<button ${count > 0 ? "x" : ""} onclick="foo()">`;')).toBe(true);
        ...
        expect(matches('const r = f() / 2; const html = `<button onclick="x()">`;')).toBe(true);
    });

    it('matches when a regex follows a block-bodied statement at top level', () => {
        ...
        expect(matches('if (x) { y(); } /`/.test(z); const html = `<button ${count > 0 ? "x" : ""} onclick="foo()">`;')).toBe(true);
        // Same pattern with other header keywords.
        expect(matches('while (x) { y(); } /`/.test(z); const html = `<button ${count > 0 ? "x" : ""} onclick="foo()">`;')).toBe(true);
        expect(matches('for (;;) { y(); } /`/.test(z); const html = `<button ${count > 0 ? "x" : ""} onclick="foo()">`;')).toBe(true);
        // `else { } /pat/` — block-introducer keyword.
        expect(matches('if (x) {} else { y(); } /`/.test(z); const html = `<button ${count > 0 ? "x" : ""} onclick="foo()">`;')).toBe(true);
        // `try { } /pat/` — block-introducer keyword.
        expect(matches('try { y(); } catch (e) {} /`/.test(z); const html = `<button ${count > 0 ? "x" : ""} onclick="foo()">`;')).toBe(true);
    });

    it('matches when a regex follows a block-bodied statement inside an interpolation', () => {
        ...
        expect(matches('`<button ${(() => { /}/.test(y); })() && count > 0 ? "x" : ""} onclick="x()">`')).toBe(true);
        // Block-introducer `try` inside the interpolation.
        expect(matches('`<button ${(() => { try {} catch(e) {} /}/.test(y); })() && count > 0 ? "x" : ""} onclick="x()">`')).toBe(true);
    });

    it('does not over-mask an object literal followed by division', () => {
        ...
        expect(matches('const x = {a: 1} / 2 / b; const html = "<button onclick=foo()>";')).toBe(true);
        // Object literal inside an interpolation, followed by division.
        expect(matches('`<button ${({a: 1}).a / 2} onclick="x()">`')).toBe(true);
    });

    it('does not match JS code that happens to contain on*=', () => {
        // No preceding `<tag`, so these are not HTML attributes.
        expect(matches('const onclick = "foo"')).toBe(false);
        expect(matches('obj.onclick = fn')).toBe(false);
        expect(matches('el.addEventListener("click", fn)')).toBe(false);
    });

    it('does not match tags without an on*= attribute', () => {
        expect(matches('<button class="x">')).toBe(false);
        expect(matches('<button title="1 > 0" id="x">')).toBe(false);
    });
});
