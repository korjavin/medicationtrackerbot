...
const ALLOWED = new Map([
    ...
    [
        'web/static/js/features/food.js:2518',
        "legacy renderFoodTargetProgress (week/2-week aggregation view) — paper-era path not targeted by Phase 4 (daily-total rewrite); slated for a follow-up phase alongside the remaining .food-target-* CSS. Line shifted after the fifth-pass review fix that recovers the v2 cache timestamp + threads staleAfterMs through to the badge.",
    ],
    [
        'web/static/js/features/food.js:2519',
        "legacy renderFoodTargetProgress (week/2-week aggregation view) — paper-era path not targeted by Phase 4 (daily-total rewrite); slated for a follow-up phase alongside the remaining .food-target-* CSS. Line shifted after the fifth-pass review fix that recovers the v2 cache timestamp + threads staleAfterMs through to the badge.",
    ],
    [
