...
const ALLOWED_GLOBALS = new Set([
    ...
    'window.DoctorBrief',               // features/brief.js — doctor-visit brief (med-5k6t.2): opens #brief-modal from the Today shortcut, builds the standalone printable HTML document from GET /api/brief, and hands it to web/cloud/js/print-doc.js for print/download. buildBriefDocument + loadPrintDoc are exposed on the same object as the test seams (no separate globals).
    ...
    'window.TrialConsent',              // features/trial-consent.js — trial-provider consent ceremony (bd med-yor.2): request(scope) modal disclosure for the ai/voice/tg scopes that PATCHes the encrypted-vault trialconsent record via /api/settings/trial-consent; retryAfterConsent(fn) is the rerun-once seam wrapped around the food AI call sites when the gate throws trial_consent_required
    ...
    'window.WGForecastCard',            // features/forecast-card.js — Gamification Phase 3 Tomorrow Forecast card on Today: fetches GET /api/gamification/forecast (cloud-only; recomputed client-side from vault bp+sleep), renders the evening in-range chance / morning resolution + the "how well do we know you" calibration meter; bootstrap.js calls .refresh() after auth, today.js .mountCard() into the rings tile per render; absent in bot mode (route 404s) and empty with the gamification feature off (shim returns {enabled:false})
    'window.Gamification',              // features/journey.js — gamification Journey screen: load() reads GET /api/gamification/journey via cachedFetch + mounts the freshness chip; render(journey) paints level/HP/streak, the rings, and the narrative layer (chapters/atlas/experiments/traits/keystones — the insight-ladder card was retired in the redesign). Gated on features.gamification (nav slot filtered before mount in bootstrap.js)
    ...
    'window.WorkoutExerciseCatalog',    // features/workout/exercise-catalog.js — single-flight catalog fetch + medical→friendly body-part translation shared by the Stats body-part split and the session-card chip (med-mj4)
    ...
    'window.WorkoutShare',              // features/workout/share.js — plan-share sender public API (share/encode/decode/buildUrl/close + makeQr/_current test seams, med-uo64.2) + import receive path (receive/openImport/closeImport + _scan live-camera lifecycle seam, med-uo64.3)
