...
export const CATALOG = [
  ...
  },
  {
    ...
    "response_summary": "Stats object with range (the echoed window), total_sessions, completed_sessions, skipped_sessions, completion_rate (percent, 0-100), active_weeks, current_streak_weeks, top_exercises[], weekly_activity[], daily_activity[], totals{volume_kg,hard_sets,easy_sets,reps,pr_count}, weekly_volume[] and exercise_totals[{exercise_name,session_count,sets,hard_sets,reps,total_volume_kg,max_weight_kg}]. weekly_activity buckets by ISO Monday; daily_activity is one entry per LOCAL calendar day inside `range` that saw a completed or skipped session (sparse — rest days are simply absent), ascending by date. top_exercises is the top-8 slice of exercise_totals (same volume math, so the two always agree); exercise_totals covers every exercise trained in the window. top_exercises/weekly_activity/daily_activity/weekly_volume/exercise_totals are null (not []) when the window holds nothing.",
    "params_schema": {
      "type": "object",
      "properties": {
        "range": {
