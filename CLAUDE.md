...
12. **A read path that lazily materializes a record into a deterministic recordId must stamp it `clientTs: 0`**, never `now()` — otherwise a device with a stale mirror re-derives that recordId in its initial state and LWW erases the real one (a confirmed dose reverting to Pending, bd med-d4w; a completed workout deleted, bd med-9a87). Ask: *could a device that hasn't synced recently create this same recordId?* If yes, it is derived state and takes the floor. The floor is not yet atomic (bd med-qhpu). It applies equally to a read/timer-side **transition** on a singleton the device may hold a stale copy of (the gamification journal + experiments, the tz plan): stamped `clientTs: 0`, the write path promotes it to `existing.clientTs + 1`, so it beats exactly the version the reader saw and never newer state it never read (bd med-y4ue). See [docs/cloud-mode.md → Sync protocol](docs/cloud-mode.md#sync-protocol) guard 3 + the incident write-up.
