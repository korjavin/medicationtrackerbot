...
10. **Device-capability access routes through `web/static/js/native/`.** Feature code calls `window.MediaCapture` / `window.Geolocation` / `window.Barcode` / `window.Reminders` rather than `navigator.geolocation`, `getUserMedia`, `BarcodeDetector`, `<input type=file capture=…>`, or Web Push directly. The abstraction picks a web or Capacitor impl at runtime via `Capacitor.isNativePlatform()`; bypassing it silently works in the browser and silently doesn't in the Android shell. New device capabilities add a paired `web/<cap>.js` + `capacitor/<cap>.js`, register via the foundation's `registerImpl`, and ship a `tests/native.<cap>.test.js`. See [docs/frontend.md → Native Platform Abstractions](docs/frontend.md#native-platform-abstractions).
11. **Mobile APK must not surface Telegram — neither remote script nor login UI.** `web/static/index.html` is shared between the server and mobile builds; it MUST NOT contain a `<script src="https://telegram.org/...">` tag. The Telegram Web App SDK is loaded dynamically by `web/static/js/core/messenger-adapter.js` (via `loadTelegramSdk()`) only when running outside Capacitor (`!window.Capacitor?.isNativePlatform?.()`) and only when `window.Telegram?.WebApp` is not already present. Additionally, the Telegram-shaped login screen (the "Login to Med Tracker" container + login-widget script builder in `app.js`) must NEVER render in the embedded shell — `checkAuth()` short-circuits on `window.__MEDTRACKER_BOOTSTRAP__?.apiBase` and goes straight to `/api/bootstrap` + `applyBootstrapPayload`, and the mobile-build `tryMobileAuthOverride` hook makes `/auth/status` return `{authenticated:true, method:"local"}`. Three architecture tests enforce this: `architecture.no-telegram-in-html.test.js` (no script tag in HTML), `architecture.mobile-no-telegram-login.test.js` (no Telegram login UI strings reachable from the embedded-shell branch in `app.js`), and `scripts/verify-apk.sh` (same no-script-in-HTML invariant on the built APK's `assets/public/index.html`). See [docs/android-emulator.md → Verifying a built APK](docs/android-emulator.md#verifying-a-built-apk).
...
- **Mobile build** — `go build -tags mobile ./...` — strips bot/MCP/web-push/OIDC at compile time, substitutes `LocalUserResolver` (single user) for the auth resolver, and uses `LocalNotificationSink` (queue + `GET /api/reminders/upcoming`) instead of `WebPushSink`. The output is a cross-compiled ELF binary that ships inside the **Capacitor Android shell** as `libmedtracker.so` under `jniLibs/<abi>/`; the shell spawns it on app launch (`-port 0`), parses `LISTENING 127.0.0.1:<port>` from stdout, and points the WebView at that port. Cross-compile pipeline + overlay sources live in `scripts/build-android-binaries.sh` + `capacitor/android-overlay/`. arm64-only without an NDK; armv7 + x86_64 are NDK-gated. iOS is deferred. See [docs/local-mode.md](docs/local-mode.md) and [capacitor/README.md](capacitor/README.md).
...
- `web/domain/` — runtime-agnostic JS domain layer for cloud mode (currently `bp.js`, `weight.js`): pure modules taking injected ports (`records`, `now`, `timeZone`), zero browser globals. Mirrors the Go domain-service contract (category/trend calculations, stats) so `web/cloud/js/apishim.js` can route `/api/bp*` + `/api/weight*` to them with no translation layer. Purity enforced by `architecture.domain-purity.test.js` — this is the seam C6 later embeds server-side via goja, so nothing here may touch `window`, `document`, `fetch`, or IndexedDB directly.
...
| E2EE cloud mode (zero-knowledge cloud + browser PWA: blind push relay, encrypted sync, per-user subdomains, BYO keys, optional Telegram, MCP relay) — **C0 (service foundation, device lifecycle, sync + push relay), C1 (BP + weight through the real `web/static` frontend via `web/domain/` + apiCall shim), C2a (diary/notes, settings incl. integrations-keys-in-vault, vitals read side), C2b (medications + intake state machine + tz-plan + reminder compute-and-upload), C2c (food logs/products/stats/meals, direct-from-browser AI parsing, direct-from-browser food-DB search with operator default), C2d (workouts: groups/variants/exercises/library CRUD, next-workout + rotation engine, session lifecycle, exercise logs, stats, mi-band read/edit), and C2e (full-vault export/import in both modes: shared Settings → Import/Export UI, `GET /api/export` + `POST /api/import` in bot mode, client-side `CloudVault` in cloud mode, optional age passphrase encryption) implemented in `cmd/cloud`; C2 is now closed** | [docs/cloud-mode.md](docs/cloud-mode.md) |
