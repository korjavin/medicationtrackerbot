...
| E2EE cloud mode (zero-knowledge cloud + browser PWA: blind push relay, encrypted sync, per-user subdomains, BYO keys, optional Telegram, MCP relay) — **C0 (service foundation, device lifecycle, sync + push relay), C1 (BP + weight through the real `web/static` frontend via `web/domain/` + apiCall shim), C2a (diary/notes, settings incl. integrations-keys-in-vault, vitals read side), C2b (medications + intake state machine + tz-plan + reminder compute-and-upload), and C2c (food logs/products/stats/meals, direct-from-browser AI parsing, direct-from-browser food-DB search with operator default) implemented in `cmd/cloud`; C2d onward still design proposal** | [docs/cloud-mode.md](docs/cloud-mode.md) |
