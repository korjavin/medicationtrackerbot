...
| E2EE cloud mode (zero-knowledge cloud + browser PWA: blind push relay, encrypted sync, per-user subdomains, BYO keys, optional Telegram, MCP relay) — **C0 (service foundation, device lifecycle, sync + push relay) and C1 (BP + weight through the real `web/static` frontend via `web/domain/` + apiCall shim) implemented in `cmd/cloud`; C2 onward still design proposal** | [docs/cloud-mode.md](docs/cloud-mode.md) |
