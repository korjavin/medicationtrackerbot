...
| E2EE cloud mode (zero-knowledge cloud + browser PWA: blind push relay, encrypted sync, per-user subdomains, BYO keys, optional Telegram, MCP relay) — **C0 (service foundation, device lifecycle, sync + push relay), C1 (BP + weight through the real `web/static` frontend via `web/domain/` + apiCall shim), and C2a (diary/notes, settings incl. integrations-keys-in-vault, vitals read side) implemented in `cmd/cloud`; C2b onward still design proposal** | [docs/cloud-mode.md](docs/cloud-mode.md) |
