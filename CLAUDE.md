...
11. **Mobile APK must not load Telegram remotely.** `web/static/index.html` is shared between the server and mobile builds; it MUST NOT contain a `<script src="https://telegram.org/...">` tag. The Telegram Web App SDK is loaded dynamically by `web/static/js/core/messenger-adapter.js` (via `loadTelegramSdk()`) only when running outside Capacitor (`!window.Capacitor?.isNativePlatform?.()`) and only when `window.Telegram?.WebApp` is not already present. The architecture test `web/static/js/tests/architecture.no-telegram-in-html.test.js` enforces the no-script-in-HTML invariant, and `scripts/verify-apk.sh` enforces the same on the built APK's bundled `assets/public/index.html` so a regression fails CI before the artifact uploads. See [docs/android-emulator.md → Verifying a built APK](docs/android-emulator.md#verifying-a-built-apk).
