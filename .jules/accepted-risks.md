...
**Finding:** When `MCP_ALLOWED_SUBJECT` is empty, the MCP server accepts any JWT that passes signature, audience, expiration, and issuer checks — i.e., any valid Pocket-ID token from the configured issuer/client. This corresponds to threat-model entry **TM-008** ("OAuth audience/subject bypass") and the recommendation in the archived bot threat model (`docs/legacy/threat-model-bot-mode.md`, TM-008) to require `MCP_ALLOWED_SUBJECT` in production.
