...
## 2026-05-10 - Fix OAuth audience/subject bypass
**Vulnerability:** Fail-open default in authorization logic (TM-008). In `isSubjectAllowed`, if the configured `AllowedSubject` was empty, the function fell back to returning `true`, allowing any valid token through regardless of subject.
**Learning:** Empty allowlists or unset authorization configurations should default to fail-closed (deny all). A fail-open fallback in critical paths like OAuth subject validation completely negates the security boundaries, letting attackers use legitimate tokens from other contexts.
**Prevention:** Always ensure authorization functions default to `return false` if the required configuration is missing or malformed, and explicitly require variables like `MCP_ALLOWED_SUBJECT` at application startup so the application refuses to start in an insecure state.
