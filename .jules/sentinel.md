...
## 2026-05-22 - MCP OAuth Subject Bypass
**Vulnerability:** OAuth audience/subject bypass (TM-008). Fail-open default.
**Learning:** In `internal/mcp/oauth.go`, the authorization check `isSubjectAllowed` defaulted to allowing any valid token from the Pocket-ID identity provider if the `MCP_ALLOWED_SUBJECT` configuration was empty. Because Pocket-ID tokens from any user account (even unassociated with the intended user) would be cryptographically valid, an empty configuration allowed any Pocket-ID user to access the MCP server's sensitive health data.
**Prevention:** Never use fail-open defaults for critical authorization boundaries. Ensure that authorization configs (like allowlists) are explicitly required during initialization (`LoadConfigFromEnv`), and that the validation logic returns `false` if the configuration is unexpectedly empty.
