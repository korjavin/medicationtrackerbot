...
## 2026-04-24 - Enforce MCP_ALLOWED_SUBJECT for OAuth Validation
**Vulnerability:** OAuth audience/subject bypass via empty allowlist (TM-008).
**Learning:** The previous implementation of `isSubjectAllowed` in `internal/mcp/oauth.go` would return `true` if the `MCP_ALLOWED_SUBJECT` configuration was empty. This insecure default meant that if an administrator failed to specify an allowed subject, the system would authorize *any* user presenting a valid token from the Pocket-ID OAuth provider, potentially allowing unauthorized data access.
**Prevention:** Remove fail-open defaults in authorization logic. Always explicitly require critical authorization configurations (like allowed subjects) at configuration load time and strictly enforce them during validation. If an allowlist is empty, it should deny access rather than granting it.
