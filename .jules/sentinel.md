...
## 2026-06-25 - OAuth audience/subject bypass
**Vulnerability:** OAuth audience/subject bypass (TM-008)
**Learning:** `MCP_ALLOWED_SUBJECT` was optional, and the validation logic defaulted to allow-all (`return true`) if the allowed subject string was empty. This meant that if the config was left out or validation was bypassed, any valid token from the Pocket-ID instance would be accepted, compromising isolation.
**Prevention:** Always require critical authorization allowlists at startup (fail-fast if missing) and ensure the runtime validation logic fails closed (e.g., `return false`) when the allowlist configuration is unexpectedly empty.
