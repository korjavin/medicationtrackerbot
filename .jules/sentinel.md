...
## 2025-05-18 - Authorization Bypass via Fail-Open Defaults
**Vulnerability:** Insecure authorization default behavior where an empty `MCP_ALLOWED_SUBJECT` allowlist granted access to any subject.
**Learning:** When implementing authorization allowlists, an empty configuration must default to fail-closed rather than fail-open.
**Prevention:** Always require critical authorization configurations (like `MCP_ALLOWED_SUBJECT`) during server initialization, and explicitly return `false` if the allowlist is somehow empty during validation checks.
