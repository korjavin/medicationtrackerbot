...
## 2025-05-19 - Fail-Open Authorization Configuration
**Vulnerability:** OAuth audience/subject bypass (TM-008). The `MCP_ALLOWED_SUBJECT` environment variable was optional, defaulting to empty, which allowed any valid Pocket-ID token from any account to access the MCP server and read health data.
**Learning:** Authorization allowlists must never default to a fail-open state. An empty or unconfigured allowlist should explicitly block all access rather than granting universal access, especially in single-user applications.
**Prevention:** Mandate critical authorization configurations (like allowed subjects) at application initialization by returning an error if they are not set, ensuring fail-closed behavior.
