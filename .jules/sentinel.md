...
## 2024-05-24 - Require MCP_ALLOWED_SUBJECT and enforce fail-closed authorization
**Vulnerability:** OAuth audience/subject bypass (TM-008). Empty `MCP_ALLOWED_SUBJECT` permitted any valid Pocket-ID token.
**Learning:** Do not use fail-open defaults (e.g., granting access if the allowlist configuration is empty). Always require critical authorization configurations at initialization and enforce fail-closed behavior during request validation.
**Prevention:** Ensure the MCP server restricts access to a specific subject during config load and validate token subjects against this allowlist safely.
