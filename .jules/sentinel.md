...
## 2025-04-20 - MCP_ALLOWED_SUBJECT Fail-Open Vulnerability
**Vulnerability:** Authorization Bypass (TM-001, TM-008)
**Learning:** The `isSubjectAllowed` function in the OAuth handler defaulted to returning `true` if `MCP_ALLOWED_SUBJECT` was empty. This fail-open design meant that if the environment variable was inadvertently omitted or misconfigured, any valid JWT token from the configured issuer could access the MCP server's health data, completely bypassing intended subject restrictions.
**Prevention:** Always enforce fail-closed authorization logic. Ensure that critical authorization parameters like allowlists return `false` or block access when empty. Additionally, validate at startup that these required configuration values are populated to prevent the application from launching in an insecure state.
