...
## 2025-03-10 - Missing Error Check on crypto/rand.Read
**Vulnerability:** The error return from `crypto/rand.Read` was being ignored during the generation of the `nonce` for session tokens in `internal/server/google_auth.go`.
**Learning:** If `rand.Read` fails (e.g., due to entropy pool exhaustion), the slice will remain zero-initialized. Ignoring this error leads to predictable nonces being used in session token creation, weakening the cryptographic security of the token.
**Prevention:** Always verify the `error` returned by `rand.Read()`. If it is non-nil, abort the operation immediately and fail securely (e.g. by returning an HTTP 500 internal server error). Do not proceed with cryptographic operations using a potentially zeroed slice.
