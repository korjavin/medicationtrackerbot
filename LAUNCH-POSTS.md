...
`{{product}}` is the product name/domain, kept out of the tree per med-9qig; substitute
the real name at publish time.
...
{{product}} rewrite on master. Remaining blockers are in the posts themselves:
