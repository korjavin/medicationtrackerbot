...
2. **Settings table** — fallback when the env var is empty.
...
FEEDBACK_ADMIN_CHAT_ID=123456789      # Optional. Numeric Telegram chat id (your own user id) the manager bot DMs when feedback arrives, so it isn't only visible to whoever remembers to run feedbackpull. Web feedback sends METADATA ONLY (kind + app version + time, no account id, no content) — the server holds client-encrypted ciphertext and stays unable to read it; telegram-origin feedback is relayed in FULL (copyMessage of the user's own message, media included) because the manager bot already held that plaintext. Requires MANAGER_BOT_TOKEN too; unset/0 = no relay, behavior unchanged. Best-effort: a failed DM (e.g. you never pressed /start on the manager bot) only logs a warning and never affects the feedback submission.
CLOUD_LOCAL_ONLY_PASSKEY_POC=1        # PROOF OF CONCEPT (bd med-eas.2.1), NOT cleared for production. Set to "1" to allow a first credential to enroll in "local-only" mode: an ordinary passkey authenticates to the server while the DEK is wrapped only by this browser's LDK and the recovery code — no server-side envelope for that credential, and no new decrypting material anywhere. The user must opt in on an explicit warning screen and must produce an Emergency Kit; the mode cannot cold-recover after site-data loss or on a fresh device. Unset (the default) = every local-only registration is rejected, GET /api/version omits the capability key, and no client shows the affordance. See docs/cloud-crypto.md "Local-only passkey (POC)".
