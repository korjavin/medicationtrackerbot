...
The `mcp_help` / `mcp_call` / `mcp_execute` trio is the long-term direction: a discover-then-run surface backed by the operation registry, instead of growing the tool list every time a new domain is exposed. `mcp_help` discovers operations (terse catalog, full drill-in, or `query` keyword search), `mcp_call` runs a single read/write directly in Go, and `mcp_execute` runs a sandboxed Python script for multi-step work. While the executor is experimental, the existing granular and composite tools stay in place — all paths are supported and tested.
...
- **Recommended path for new MCP clients:** start with `mcp_help` to discover operations (the catalog is terse; pass `query=` to search or `operation_id=`/`topic=` to see schemas + an example), then run a single read/write with `mcp_call` or compose a multi-step workflow with `mcp_execute`. Fall back to the granular tools for clients without script execution support, and for the two composite analyses listed above.
