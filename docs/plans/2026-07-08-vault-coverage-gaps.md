...
- [x] `internal/server/vault_coverage_test.go`: a declared `vaultCovered` set + `vaultSkipped` map (table → Reason), asserted against the table list `WipeUserTx` actually deletes from. Export the wipe manifest from `seeddemo` (a `var WipedTables []string` next to the existing lists, or a small accessor) rather than re-typing it. Fail with the exact missing table name in either direction. ⚠️ `seeddemo` exposes `WipedTables()` (accessor over four package-level manifests — the raw-SQL delete loops were re-shaped into `{table, query}` pairs so the manifest is derived from what actually runs, not re-typed beside it). Guard checks four directions: wiped⊆covered∪skipped, covered⊆wiped∪`vaultCoveredNotWiped`, no stale skip entries, and every named table exists in the migrated schema (typo guard).
- [x] Skip-list Reasons: `push_subscriptions` (device-bound), `intake_reminders` (Telegram message ids), `change_events` (SSE tag stream), `miband_gps_tracks` (nested under `workouts.miband[].gps`), `workout_schedule_snapshots` (write-only; `ListGroupSnapshots` has no callers), `weight_goals`/etc. (already covered — these belong in `vaultCovered`). ⚠️ `miband_gps_tracks` went into `vaultCovered`, not the skip list — it *is* carried (nested under its workout), so "covered" is the honest label. Added `vaultCoveredNotWiped` (`settings`, `api_tokens`) with reasons: both are carried but deliberately not wiped, and the reverse-direction check would otherwise flag them. ✅ `TZ=UTC go test ./internal/server ./internal/seeddemo` green; verified the guard fails with the exact table name by removing `gamification_ledger` from `vaultCovered`.
