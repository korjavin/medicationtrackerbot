...
- [ ] add `TopUpOpts { UserID int64, DBPath string, Now time.Time, Seed int64 }` and `TopUp(ctx, opts) (Summary, error)`.
- [ ] for each stream (BP, weight, sleep, food, workouts, diary, HR, SpO2, stress), call the corresponding `store.Latest…` to find `lastTs`; if `!found`, fall back to `now.AddDate(0,0,-1)` (one day of backfill is enough — full seeding is a separate command).
- [ ] derive a per-tick RNG seed `pcg(uint64(opts.Seed) XOR uint64(opts.Now.Unix()/86400))` so re-running the same tick produces identical samples (idempotent on retry).
- [ ] for meds: do **not** touch the medication catalog. Iterate existing medication rows for the user; for each one whose active date range covers `lastIntake..now`, append intake_log rows forward (re-using the existing intake-generator logic). Decision recorded in planning: keep windows fixed.
- [ ] write tests covering: (a) empty DB top-up generates a 1-day backfill, (b) seeded DB top-up generates rows strictly after the prior max, (c) running top-up twice in a row is a no-op for streams whose interval doesn't divide the gap, (d) HR/SpO2/stress UNIQUE PK prevents duplicates even on same-second retry.
- [ ] run `go test ./internal/seeddemo/...` — must pass before task 5.
