...
- [ ] create `internal/mcp/catalogjs/catalogjs.go`, package `catalogjs`, importing only stdlib + `internal/mcp/registry`
- [ ] define `type Exclusion struct { ID, Reason string }` and `var Excluded = []Exclusion{…}` listing the **8 gamification op ids** individually (not the topic), each with reason `"gamification is deferred project-wide; clamped out of apishim.js PORTED_SET"` — mirror `internal/server/mcp_coverage_exempt.go`'s reasoned-entry shape
- [ ] add `func ExcludedIDs() map[string]string` returning id→reason for cheap lookup
- [ ] implement `func Generate(ops []*registry.Operation) ([]byte, error)` that filters out excluded ids, sorts the remainder by `ID` (deterministic, diff-stable), and renders a browser-ESM file
- [ ] emit `export const CATALOG = [...]` where each entry carries `id, topic, method, path, path_params, risk, description, response_summary, params_schema, body_schema, response_example, required` — marshal `ParamsSchema`/`BodySchema` (`json.RawMessage`) as inline JSON objects, omitting them when nil
- [ ] compute `required` in Go (path params + required field names from params/body schemas) and bake it into each entry, so the JS side never has to walk a JSON Schema — reuse the same derivation `registry.MarshalForHelpCompact` uses rather than reimplementing it
- [ ] emit `export const EXCLUDED = [{ id, reason }, …]` from `Excluded`
- [ ] prepend a `// Code generated by cmd/genmcpcatalog. DO NOT EDIT.` header plus a one-line pointer to this plan, and ensure the output has **no `import` statements** and ends with a trailing newline

### Task 2: Add the `cmd/genmcpcatalog` entry point and check in the generated catalog
- [ ] create `cmd/genmcpcatalog/main.go` as a thin `main`: call `catalogjs.Generate(registry.DefaultOperations())`, write to `web/cloud/js/mcp-catalog.generated.js`, accept an optional `-out` flag defaulting to that path
- [ ] add a `//go:generate go run ./cmd/genmcpcatalog` directive in `catalogjs.go` so the regeneration command is discoverable from the package that owns the format
- [ ] run the generator and commit `web/cloud/js/mcp-catalog.generated.js`
- [ ] confirm `go build ./...` still passes (the file is embedded by `web/cloud/embed.go`'s existing `js` directory glob — verify no embed edit is required)
- [ ] sanity-check the emitted op count is `106 - 8 = 98` and that no gamification id appears in `CATALOG`
