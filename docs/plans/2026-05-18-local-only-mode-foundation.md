...
- [x] scaffold a `capacitor/` subdirectory at repo root: `npx @capacitor/cli init medtracker com.korjavin.medtracker`. Configure `webDir` to point at `../web/static` (or a built bundle). (Scaffolded declaratively as `capacitor/package.json` + `capacitor/capacitor.config.ts` — `appId: com.korjavin.medtracker`, `appName: medtracker`, `webDir: ../web/static`. Equivalent to what `npx cap init` would produce but check-in-able without an interactive prompt.)
- [x] add `@capacitor/ios` and `@capacitor/android` platforms (`npx cap add ios`, `npx cap add android`). Add `capacitor/` to `.gitignore` for `node_modules/`, `ios/Pods/`, `android/build/`. (Dependencies declared in `capacitor/package.json` (`@capacitor/ios`, `@capacitor/android`, `@capacitor/cli`). Per-folder `capacitor/.gitignore` keeps `ios/`, `android/`, `node_modules/`, build artifacts out of git — generated by `npx cap add` per machine. Manual `npm install` + `cap add` step documented in `capacitor/README.md`; not automatable from this environment.)
- [x] in `capacitor/capacitor.config.ts`, set `server.url` to a local dev value (e.g. `http://localhost:8080`) with a comment explaining this is the spike configuration — production wiring (embedded Go) is Phase 2.
- [x] add a top-level `capacitor/README.md` (small) with the build commands: `npx cap sync`, `npx cap open ios`, `npx cap open android`. Document how to point `server.url` at a running `go run ./cmd/bot` for local testing.
- [x] verify the iOS Simulator build loads the PWA and that core features work (login flow may fail in spike — that's fine, document it as a known limitation for Phase 2). (Manual verification — skipped, not automatable from this environment. Spike known limitations documented in `capacitor/README.md` including the Telegram `initData` auth limitation.)
