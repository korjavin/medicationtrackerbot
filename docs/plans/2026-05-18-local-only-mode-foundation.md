...
- [x] scaffold a `capacitor/` subdirectory at repo root: `npx @capacitor/cli init medtracker com.lochyard.medtracker`. Configure `webDir` to point at `../web/static` (or a built bundle). (Scaffolded declaratively as `capacitor/package.json` + `capacitor/capacitor.config.ts` — `appId: com.lochyard.medtracker`, `appName: medtracker`, `webDir: ../web/static`. Equivalent to what `npx cap init` would produce but check-in-able without an interactive prompt.)
