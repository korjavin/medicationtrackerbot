...
5. Notifications use the same notifier as recurring workouts; the notification body lists the planned exercises. The advance window is fixed at 0 minutes — notify at the scheduled time exactly, since the user already picked the moment. No per-session `advance_minutes` field is exposed (callers wanting an earlier reminder should set `scheduled_time` accordingly).
