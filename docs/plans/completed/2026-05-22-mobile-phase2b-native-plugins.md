...
- [x] create `web/static/js/native/web/reminders.js` exporting `schedule(reminders)` and `cancelAll()` as no-ops on web (or thin shims that document "Web Push is handled by `push.js`; this is a Capacitor-only path"). Web callers continue to use the existing webpush subscribe flow.
- [x] create `web/static/js/native/capacitor/reminders.js` calling `@capacitor/local-notifications`. `schedule(reminders)` is **replace-all**: calls `LocalNotifications.getPending()`, cancels all pending with `LocalNotifications.cancel(ids)`, then schedules the new batch. Resolves the stub's open question (simpler, no diff bookkeeping; risk of duplicate notifications is the cost we accept). `cancelAll()` cancels every pending. Each reminder is mapped to `{ id: intake_id, title: medication_name, body: 'Time to take...', schedule: { at: new Date(scheduled_at) }, extra: { intake_id, medication_id } }`. (Reads plugins via `window.Capacitor.Plugins.LocalNotifications` and `window.Capacitor.Plugins.App` to match the geolocation / media-capture / barcode pattern — no JS bundler required.)
- [x] update `web/static/js/native/index.js` to wire `window.Reminders`. (Wiring is handled by the foundation's `registerImpl` helper that the new impl files call into; index.js itself is unchanged from Task 2. New script tags for `web/reminders.js` and `capacitor/reminders.js` added to `web/static/index.html` and to the service worker's `STATIC_ASSETS` precache list.)
- [x] create the pre-schedule loop in `web/static/js/native/capacitor/reminders.js`: `startPreScheduleLoop()` that (1) immediately polls `GET /api/reminders/upcoming?hours=24`, (2) calls `Reminders.schedule(response)`, (3) registers a Capacitor `App.addListener('appStateChange', ...)` so the same flow re-runs on resume. Loop is started by the foundation module at app-ready time when on native platform. (Idempotent — calling `startPreScheduleLoop()` twice does not register listeners twice. The actual app-ready boot invocation is left to Task 6 / Task 7 wiring; the function is exposed on `window.Reminders.startPreScheduleLoop`.)
- [x] add an `App.addListener('appStateChange', ...)` deep-link handler in `web/static/js/native/capacitor/reminders.js` (or a small `notifications-tap.js` sibling) that, when a notification tap delivers `extra.intake_id`, calls into the existing app routing to deep-link to the today/medications view for that intake. Reuses whatever navigation surface `web/static/js/app.js` already exposes — no new router work. (Listens on `localNotificationActionPerformed`; sets a `?action=medication_confirm&ids=<med>&intake_ids=<intake>` query string and calls `window.handleDeepLinks()` so the existing `handlePushAction` path in app.js opens the medication-confirm modal — same routing surface push.js uses on web.)
- [x] write `web/static/js/tests/native.reminders.test.js`: web `schedule()` is a no-op (does not throw), Capacitor `schedule()` cancels pending before scheduling new (replace-all semantics), Capacitor `schedule()` maps endpoint response to plugin payload shape correctly, Capacitor `cancelAll()` calls `LocalNotifications.cancel`, pre-schedule loop fires on `appStateChange` to active, deep-link handler invokes the app navigation surface with the intake id from `extra`. Also covers: schedule() drops entries missing intake_id / invalid scheduled_at, empty-name fallback to "Medication", permission-denied normalization to RemindersError/PERMISSION_DENIED, UNAVAILABLE when plugin is missing, `_refreshFromServer` prefers `offlineAwareApiCall` over `fetch`, fetch errors are swallowed (best-effort), `startPreScheduleLoop` is idempotent, deep-link handler is a no-op when extra.intake_id is missing, runtime selector picks web/capacitor based on `isNativePlatform()`.
- [x] run `pnpm test` — must pass before Task 6.

### Task 6: Capacitor plugin install + Android overlay wiring

- [x] add `@capacitor/camera`, `@capacitor/geolocation`, `@capacitor-mlkit/barcode-scanning`, `@capacitor/local-notifications` to `capacitor/package.json` dependencies at their latest 6.x-compatible versions (Capacitor core is `^6.1.2`). (Pinned to `@capacitor/camera@^6.1.0`, `@capacitor/geolocation@^6.1.0`, `@capacitor/local-notifications@^6.1.0`, `@capacitor-mlkit/barcode-scanning@^6.2.0` — all 6.x-compatible.)
- [x] add `<uses-permission>` entries to `capacitor/android-overlay/app/src/main/AndroidManifest.xml` for `CAMERA`, `ACCESS_COARSE_LOCATION`, `POST_NOTIFICATIONS`, `SCHEDULE_EXACT_ALARM`. Also add the `<uses-feature android:name="android.hardware.camera" android:required="false" />` and barcode-scanner manifest metadata per `@capacitor-mlkit/barcode-scanning` README. (POST_NOTIFICATIONS was already present from Phase 2a's service notification — same permission covers both. Added MLKit `<meta-data com.google.mlkit.vision.DEPENDENCIES value="barcode">` to bundle the barcode model at install time so the first scan doesn't trigger a Play Services model download.)
- [x] add `LocalNotifications` plugin config block to `capacitor/capacitor.config.ts` (icon, default sound, small-icon resource name). (Added `plugins.LocalNotifications = { smallIcon: 'ic_launcher', iconColor: '#2481cc', sound: undefined }` — ic_launcher is the fallback until a dedicated white-on-transparent notification icon is added under res/drawable.)
- [x] update `capacitor/README.md` with a "Phase 2b plugins" section: which plugins are wired, what AndroidManifest permissions they require, what to run after pulling (`npm install` + `npx cap sync` + `./apply-overlay.sh`). (Section also includes a real-device verification checklist for food photo / barcode / reminders.)
- [x] write a Vitest test in `web/static/js/tests/native.foundation.test.js` (extend existing file from Task 1) asserting that after the full module load (all four abstractions wired), the four globals are functional methods, not stubs. (Added describe block "full module load wires real impls" with three tests: web path, capacitor path, plus a check that `window.Reminders.startPreScheduleLoop` is exposed on the capacitor path. Uses a `callAndSwallow` helper to consume the promise return values so plugin-not-available rejections don't surface as test-runner unhandled rejections.)
- [x] run `pnpm test` — must pass before Task 7. (All 223 test files / 2432 tests green, including the 11 tests in native.foundation.test.js.)

### Task 7: Refactor callers to use abstractions

- [x] update `web/static/js/features/food/scanner.js`: replace lines 40–64 (BarcodeDetector creation) + line 102 (`detector.detect`) + line 193 (ZXing fallback) + line 216–258 (`openPhotoPickerAndDecode`) with calls to `window.Barcode.scan()` and `window.MediaCapture.pickPhoto()`. Remove the now-dead code paths. (Removed `createFoodBarcodeDetector`, `_getDetector`/`_setDetector`, `decodeBarcodeFromImageFallback`, `decodeFromImageWithDetector`. `scanFrameLoop` now calls `window.Barcode.scan({ source: video, formats })`; `openPhotoPickerAndDecode` calls `window.MediaCapture.pickPhoto()` followed by `window.Barcode.scan({ source: file, formats })`. The modal's `startFoodScanner` still calls `getUserMedia` directly for the live camera stream — that's the web-side UX which the abstraction's `source: video` parameter is designed to plug into.)
- [x] update `web/static/js/features/food/photo.js`: replace `triggerFoodPhotoPicker` (line 19–24) with a call to `window.MediaCapture.takePhoto()` (or `pickPhoto()` for gallery selection — preserve whichever the existing UI offers). Keep the EXIF parsing at line 154–161 and the `POST /api/food/log/from-photo` at line 208 unchanged. (Picked `pickPhoto({ capture: false })` because the existing static `#food-photo-input` omits the `capture` attribute — the food picker must allow both camera and gallery selection. Extracted `uploadFoodPhotoFile(file)` so the abstraction path and the legacy `uploadFoodPhoto(input)` change-handler path both feed into the same EXIF + POST + cache-invalidation pipeline. The static input + its change handler remain wired as a fallback surface.)
- [x] verify `web/static/js/features/bootstrap.js`'s tz detection (line 22) does NOT change — Intl is the right answer for the tz string and the Geolocation abstraction is for a future capability, not this refactor. (Confirmed: `bootstrap.js:22` still uses `Intl.DateTimeFormat().resolvedOptions().timeZone`.)
- [x] update or extend `web/static/js/tests/features.food.scanner.test.js` and `features.food.photo.test.js` so the integration assertions go through the abstraction seam (mock `window.Barcode` / `window.MediaCapture` instead of `navigator.mediaDevices`). (Updated `features.food-photo.test.js`'s "triggerFoodPhotoPicker" case to assert `window.MediaCapture.pickPhoto({ capture: false })` is called; updated `food.actions-photo-picker.test.js`'s click-based assertions to MediaCapture-based assertions; updated `app.visual-and-scanner.test.js`'s "scanner helpers" case to drop the now-removed `createFoodBarcodeDetector` assertions while preserving the `sanitizeScannedValue` / `handleDecodedValue` coverage.)
- [x] write tests for the refactored callers: scanner uses `window.Barcode.scan()` and dispatches the existing barcode-found event; photo flow uses `window.MediaCapture.takePhoto()` and posts the resulting blob to `/api/food/log/from-photo` unchanged. (New `features.food-scanner-abstraction.test.js`: 6 cases covering `openPhotoPickerAndDecode` happy path, pick-cancelled, decode-returns-null, decode-error, `scanFrameLoop` per-tick `Barcode.scan` call, and error-swallow + loop re-arm. New `features.food-photo-abstraction.test.js`: 6 cases covering `triggerFoodPhotoPicker` → `MediaCapture.pickPhoto`, picked-file POST pipeline, pick-cancelled no-op, pick-rejection swallow, defensive bail when `window.MediaCapture` is absent, and the `uploadFoodPhotoFile` export.)
- [x] run `pnpm test` — must pass before Task 8. (All 225 test files / 2444 tests green, including the 12 new tests and the updated existing ones. Frontend harness now loads `native/index.js` + `native/web/*.js` before feature modules so the `window.Barcode` / `window.MediaCapture` globals are real impls — not stubs — when the refactored callers evaluate.)
...
- [x] update `docs/local-mode.md`'s "Native plugin JS abstractions" subsection: mark Phase 2b as in-progress / shipped (per actual landing), describe the `web/static/js/native/` runtime selector pattern, link to this plan. (Status header now flags Phase 2b as shipped (real-device verification pending), the Phase 2 sub-section bullet for Phase 2b links to this plan, and the "Native plugin JS abstractions" subsection rewrites the capability table to include the global name + Capacitor impl details, then documents the runtime selector pattern, the reminder pre-schedule loop with replace-all semantics, and the refactor footprint.)
- [x] update `docs/frontend.md` if a native-abstractions section makes sense there (it probably does — the abstraction is part of the frontend architecture). Add a "Native platform abstractions" subsection that names the four globals, the runtime selector, and where the web vs Capacitor impls live. (Added a "Native Platform Abstractions" subsection above "SW Cache Strategy" covering the four globals, the runtime selector pattern, the pre-schedule loop, the refactor footprint, and a "How to add a new device capability" checklist. The four globals were also added to the Global Namespace Policy table.)
- [x] update `capacitor/README.md` (already touched in Task 6) with a verification checklist for someone pulling Phase 2b for the first time: `npm install`, `npx cap sync`, `./apply-overlay.sh`, then `npx cap open android`. (Already complete from Task 6 — verified the "Setup after pulling Phase 2b on a fresh checkout" block + the real-device "Verification checklist" cover all the requested steps.)
- [x] add a CLAUDE.md note (only if the abstraction pattern needs to be enforced going forward — e.g. "new device-API access must route through `web/static/js/native/`") — judgment call during the task; skip if the pattern feels too narrow to deserve a top-level rule. (Added — the failure mode is silent web/Capacitor divergence in feature code, which is exactly the class of bug CLAUDE.md rules exist to prevent. New rule #10 names the four globals, points to the foundation pattern for adding new capabilities, and links to the new frontend.md subsection.)
