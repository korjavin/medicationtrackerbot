...
- [x] update `docs/local-mode.md`'s status header: mark Phase 2c as shipped (real-device verification pending), link to this plan. Update the Phase 2 sub-section. (Status banner now lists Phase 2c shipped with the real-device caveat and points at this plan; the Phase 2 sub-section list now has a Phase 2c "shipped" entry and a new Phase 2d "stub, deferred" entry. The "Open questions for Phase 2" list also flips question #2 from "punt" to "resolved by Phase 2c".)
- [x] rewrite `docs/local-mode.md`'s "First-run Settings flow" subsection to describe the shipped flow (4 screens, fully skippable, the `needs_first_run` bootstrap field, the `POST /api/firstrun/complete` endpoint). (Replaces the previous 5-line "must configure OpenAI" sketch with a description of the four screens, the bootstrap-field trigger, the idempotent completion endpoint, the sessionStorage-based resume safety, and the `__firstRun` vs `needs_first_run` naming-collision guard.)
- [x] rewrite `docs/local-mode.md`'s "First-run user provisioning" subsection to describe the lazy provisioning in `handleFirstRunComplete`. (Captures the Phase 2c discovery that the schema has no `users` table — provisioning is implicit via the first write under `user_id = N` — so the planned INSERT OR IGNORE step was intentionally removed and the endpoint comment now documents why.)
- [x] update `docs/local-mode.md`'s "Secrets storage" subsection: capture the SQLite-plaintext decision with a written threat-model justification (single-user device, no network exposure, defense-in-depth deferred to Phase 2d). Link to the new Phase 2d stub. (Adds a "Phase 2c decision (shipped)" block with the threat model (attack vectors a–d, why Keystore doesn't help on a single-user device), the `android:allowBackup="false"` orthogonal note, and a "Phase 2d (deferred)" block sketching the EncryptedSharedPreferences + header-forwarding migration with a one-week real-device baking gate.)
- [x] update `cmd/bot/main_mobile.go`'s top-of-file comment: remove the "The first-run experience is Phase 2 work" line (it's now shipped) and replace with a pointer to this plan. (The old "Phase 1 expects the DB to be pre-seeded or for the user to walk through the in-app Settings screen after first launch" sentence is replaced with a description of the four-screen overlay, the bootstrap trigger, and references to this plan and `docs/local-mode.md`'s "First-run Settings flow" subsection.)
- [x] delete the old stub `docs/plans/2026-05-22-mobile-phase2c-firstrun-secrets.md` (superseded by this plan). (Removed via `git rm`. The completed Phase 2b plan still forward-references the old path, which is acceptable since completed plans are frozen historical artifacts; the current plan's own references to the old stub describe the supersession and remain accurate.)
- [x] create `docs/plans/2026-05-23-mobile-phase2d-keystore-secrets.md` as a Status: Stub follow-up plan capturing the EncryptedSharedPreferences/Keystore migration, with the deferred decision rationale and the `frontend → fetch secret → POST init → Go uses it` ceremony sketch from the stub's secrets discussion. (Created with: Status (deferred until Phase 2c has baked on real device for ≥1 week), Overview, Goals (per-secret EncryptedSharedPreferences entries, `MedtrackerNative.getProviderSecret` bridge, frontend cache, header-forwarding on AI requests, migration path that wipes SQLite values after confirmation), Out of scope (iOS-specific Keychain wiring, per-secret encryption tiers, multi-profile namespacing), Approach sketch (6 steps), Risks (bridge fragility, header leakage, migration race, server-build regression), Estimate, and Open questions.)
- [x] consider a CLAUDE.md note: "new install bootstrap surfaces must check `needs_first_run` rather than rendering directly into an empty DB". Judgment call — skip if the pattern is too narrow to justify a top-level rule. (Skipped — `needs_first_run` is a single bootstrap field consumed by a single overlay module; the pattern is too narrow to belong in CLAUDE.md alongside the existing high-altitude rules (domain service pattern, design tokens, etc.). The docs/local-mode.md "First-run Settings flow" subsection is the right home for this guidance.)
