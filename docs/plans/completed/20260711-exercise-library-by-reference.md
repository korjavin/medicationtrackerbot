...
- [x] add a one-time, idempotent boot migration (after unlock, `web/cloud/js/cloud-boot.js`) that backfills `exercise_library_id` refs on pre-migration `workoutexercise` records. **Scope change vs plan**: dropped the `importAll` snapshot shape (`markForceSnapshotPending → dropPendingForTypes → replaceAllRecords → forceSnapshot`) — that path is for a *full-vault replace* and would drop tombstones / require the `isBootstrapped` guard. This backfill is purely additive, so it reuses the normal exercise-update path via `shimCall('/api/workout/exercises/update…')`, which already dedups by name into the library and sets `exercise_library_id` exactly like `createExercise`. Guarded by construction: it filters to non-blank-name `workoutexercise` records with a null `exercise_library_id`; a rerun finds zero and no-ops (no separate "has run" flag).
...
- [x] in the library edit modal (`web/static/js/features/workout/library.js`), add a short note that renaming updates the exercise everywhere it's used in plans (history logs snapshot the name at log time and are unaffected) — hint element in `index.html` (`#exercise-library-rename-hint`), shown only in edit mode, hidden in add mode; `.wg-workouts-library-modal__hint` token-based style
...
- Rename an exercise in the library UI (both modes); confirm the new name shows in existing plans. (Past session history intentionally keeps the name it was logged under.)
