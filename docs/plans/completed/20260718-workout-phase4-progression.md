...
- [x] **Floor drift:** test `progression double: a manual exercise edit (payload omits the window) preserves the anchored floor` expects `target_reps_min === 8` but gets `11`. Root cause was NOT the anchor path (that already preserves the stored floor of 8) — it was `propagateExerciseToSchedule`'s name-match guard: the manual edit renames the exercise ('Bench'→'Bench Press'), leaving the log's cached `exercise_name` stale, so the guard `exercise.exercise_name !== exerciseName` wrongly no-op'd the final reset. Dropped the name check from the guard (Go keeps it only to defend against cross-table id collisions, which cannot occur in the type-scoped cloud records model — `findByNumericId` is EXERCISE-scoped); now matches by id + variant only. `web/domain/workout.js:~1317`.
- [x] **Invalid rule must reject, not resolve null:** tests `progression double: inverted rep targets (max < min) with an implicit window are rejected` and `progression: rejects an out-of-range increment_kg that would overflow the plan weight`. The domain already throws `invalidRequest` (code `'invalid_request'`) correctly; the swallow was in `core/api.js` `apiCall`, which caught → `console.error` → returned `null`. Made `apiCall` propagate client-side validation rejections (`e.code === 'invalid_request'`) in both delegation branches, like it already rethrows aborts — server-origin 4xx carry only `.status` (no such code) so bot mode is unaffected. Non-finite `increment_kg` still clamps to 2.5 via `numOrNull` (returns null → default) rather than throwing; out-of-range/inverted reject — all three tests pass. Also updated the stale bot-mode `saveExercise` unit test (`workout.crud.test.js`) whose expected payload predated Phase 4's always-sent `progression_rule:{type:'none'}` key.
- [x] Run `web/static/js/tests/cloud.shim-contract.workout-sessions.test.js` + the full frontend suite with Node 20 — 0 failures, 0 unhandled errors — before finalizing. (target file 35/35; full suite 317 files, 3768 passed, 29 skipped, 0 failures, EXIT=0)
