...
- [x] keep the existing `aggregateToday()` in `web/static/js/features/today.js` unchanged — its contract is reused
- [x] rewrite `renderToday(state)` in `today.js` to produce the new markup: sun-accent next-action card → 2×2 vitals tile grid (`MetricTile`) → fuel-today card with mini-bars (`MiniBar`) → workout+sleep two-up → consistency streak card. Structure matches `screens.jsx:6-124` but adapted to the actual aggregator fields: the vitals grid shows the two metrics the aggregator produces (BP + Weight) in place of the prototype's four tiles (BP / heart rate / SpO₂ / weight), since the backend does not track HR or SpO₂. Workout + Sleep live in the plan grid, the streak card is a placeholder ("Consistency tracking coming soon") because streak computation is not in scope for the reskin.
- [x] extract `renderMetricTile({ label, value, unit, statusTag, sparkPoints, variant })` and `renderMiniBar({ label, pct, variant })` as private helpers in `today.js` — color values come from `--wg-*` tokens via class names (`wg-sparkline--sun` / `--mint` / `--mint-soft` / `--coral`, `wg-mini-bar__fill--sun` / `--mint` / `--sage` / `--sun-deep`), NOT inline `stroke=`/`fill=`. Mini-bar widths ride on an SVG `<rect width="…">` attribute rather than an inline style — this keeps the `[style]` attribute off the DOM and satisfies the existing no-inline-styles architecture test.
- [x] add CSS: `.wg-next-action-card`, `.wg-vitals-grid` (2-col grid, 8px gap via `var(--space-sm)`), `.wg-metric-tile` + `__label`/`__value`/`__unit`/`__spark`/`__status`, `.wg-fuel-card` + `__header`/`__total`/`__pct`/`__bars`, `.wg-mini-bar` + `.wg-mini-bar__track` + `.wg-mini-bar__svg` + `.wg-mini-bar__fill` (+ `--sun`/`--mint`/`--sage`/`--sun-deep` variants), `.wg-plan-grid`, `.wg-plan-tile`, `.wg-streak-card`, `.wg-streak-bars`, plus sparkline primitives `.wg-sparkline`/`--sun`/`--mint`/`--mint-soft`/`--coral`, `.wg-spark`, `.wg-spark-fill`, `.wg-spark-tail`. Added 20 new `--wg-*` tokens (layout + font-size + gradient + shadow strings) to `:root` and wired them into `WANDERGEEK_TOKENS` in `architecture.design-tokens.test.js`.
- [x] port the `Sparkline` component to `web/static/js/components/wg-sparkline.js` — `WGSparkline.render({ points, variant, width, height })` returns an `<svg>` whose variant class lives on the element itself (stroke & fill colours cascade through `currentColor` from a single per-variant `color:` CSS property). Stroke, fill, and tail-circle colours all track the same token; the `<path>` has no inline `stroke=` attribute.
- [x] register `window.WGSparkline` in `architecture.globals.test.js` — added alongside `window.WGBottomNav` and `window.WGIcons` with a one-line justification.
- [x] update existing `today.render.test.js` snapshot — rewrote the whole file against the new markup (old `.today-card*` assertions replaced by `.wg-next-action-card`, `.wg-metric-tile`, `.wg-fuel-card`, `.wg-mini-bar`, `.wg-plan-tile`, `.wg-streak-card`). Dropped the `cardOrder` test — layout order in the new design is fixed (next-action → vitals → fuel → plan → streak), not a saved preference. Kept offline-banner, first-run, disabled-all, header-with-gear, and onDeeplink/onSettings coverage.
- [x] write `today.render.wg.test.js` — 9 new characterisation tests covering: canonical section ordering (data-section attributes), metric-tile routing (BP → 'bp', Weight → 'weight'), fuel card routing ('food'), next-action routing ('meds'), plan tiles routing ('workouts' and 'health'), mini-bar macro count (4) + energy-bar SVG width (matches kcal/target pct) + no-inline-style guard, sparkline variant wiring (BP → sun, Weight → mint-soft), 14-bar streak placeholder, and no-target fuel state ("—" + "No target set"). Matches the plan's "'bp'/'bp'/'more'/'more'" intent, adapted to the two-metric vitals grid we actually render.
- [x] write `components.wg-sparkline.test.js` — 9 tests: SVG namespace + width/height/viewBox, variant class on `<svg>`, deterministic `d` attribute for a 4-point series, fill path shape + variant class, tail circle at last point, null for empty/invalid input, no fill path for single-point data, no inline stroke on `<path>`, default width/height when unspecified.
- [x] run `pnpm test` — all 585 tests pass across 66 files; `go test ./...` green (sanity check, no backend changes).
