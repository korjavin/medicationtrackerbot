...
- [ ] create `web/static/js/features/meds-history.js` containing the
...
- [ ] convert any module-level state to closure-private `_state`
- [ ] delete the moved code from `app.js`; leave one-line delegation
  shims only where an existing global name must keep working
- [ ] add the file to `index.html` (before `app.js`) and to `sw.js`
  `STATIC_ASSETS`
- [ ] add `window.MedsHistory` to the `architecture.globals.test.js`
  allowlist with justification
- [ ] extend the meds feature suite (via frontend harness) covering:
