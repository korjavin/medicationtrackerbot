...
- [x] `ManagerWebhook` (`telegram.go:451`): add an `upd.CallbackQuery != nil` branch →
...
- [x] Offer the button: when `feedbackAgeRecipient != ""` **and**
...
- [x] Capture path in `handleManagerMessage` (before the greeting switch): if
...
- [x] Tests (copy `TestManagerOnboarding` + child-callback JSON): (a) a greeting from a
