...
- [x] Verify all Overview goals are implemented: (a) Today's Next Medication tile renders from cache when bootstrap is unreachable; (b) Food today shows cached log offline; (c) every priority section has a stale badge; (d) no new offline write paths were introduced; (e) cold-start behavior unchanged. (a) `loadNextIntakeCached` at `app.js:1106` with `today.next-intake-cached.test.js` (4 cases). (b) `loadFoodLogs` wrap at `food.js:1814` with `food.offline-cached-fetch.test.js` (7 cases). (c) Today/Food/BP/Weight/Meds×2 (schedule + history) / Workouts×2 (history + groups) / Vitals×2 (overview + notes) all mount the badge — coverage in `today.stale-badge.test.js`, `food.stale-badge.test.js`, and `sections.stale-badge.test.js` (8 cases). (d) `git diff master..HEAD --stat` shows only frontend read-side changes — no new outbox/queue endpoints. (e) Cold-start unchanged: `cachedFetch` throws `OfflineNoCacheError`, every consumer (`app.js:1117`, `food.js:276`, `food.js:1845`) catches it and renders an explicit empty state instead of crashing.
- [x] Verify `OfflineNoCacheError` is handled (not thrown to console) in every section that uses `cachedFetch`. The only `cachedFetch` consumers are `app.js:1106` (Today next_intake), `food.js:259` (products), `food.js:1814` (daily log) — each has a matching `instanceof window.OfflineNoCacheError` branch (`app.js:1117`, `food.js:276`, `food.js:1845`). Other sections (BP/Weight/Meds/Workouts/Vitals) only mount the badge via `WGStaleBadge.mountFromKey` and keep their existing `offlineAwareApiCall` read paths, so they cannot raise `OfflineNoCacheError`.
- [x] Run full Vitest suite `pnpm test` — all green. 1697 / 1699 passing. The 2 failures (`components.wg-sleep-chart.test.js`, `components.wg-steps-chart.test.js` — "Today" x-axis label) reproduce on master with no resilience changes applied (verified via `git checkout master && npx vitest run …`). Date-dependent: tests assume the chart's last column relabels to "Today" but today is Sunday 2026-05-09, and the labels render `Sun` instead. Pre-existing, documented as such in Tasks 2, 3, 4, 5, 6.
- [x] Run `go test ./...` (sanity — no backend changes expected, but the architecture/coverage tests must still pass). All Go packages green (`internal/server`, `internal/store`, `internal/mcp/...`, etc.). Backend was untouched on this branch.
- [x] Verify no new inline-style or globals violations (`tests/architecture.*.test.js`). `architecture.globals.test.js`, `architecture.inline-styles.test.js`, `architecture.design-tokens.test.js`, `architecture.sw-precache.test.js`, `architecture.wg-primitives.test.js`, `architecture.toolbar-btn.test.js`, `architecture.chart-theme.test.js` all pass. The new `window.WGStaleBadge` and `window.OfflineNoCacheError` globals are explicitly entered in the allowlist with justifications; the badge component uses `--wg-*` tokens and CSS classes only (no inline `.style.` assignments).
- [x] Verify no Dexie schema version bump was required, OR that the bump is correct and migration is additive only. `git diff master..HEAD -- web/static/js/db.js` returns no output — `db.js` is byte-identical to master. The pre-existing `api_cache` store was reused as-is (no schema bump needed; `cacheApiSnapshot` already wrote `{ id, timestamp, data, tags }`).
