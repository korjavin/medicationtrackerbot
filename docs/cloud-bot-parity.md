...
1. **med-eas.56** (P2) — Composite MCP `analyze_*` → client-side cloud implementation; cloud's substitute for the unavailable `mcp_execute`. *(done — Path B, cloud-only catalog seam; see the composite-analysis parity row above)*
