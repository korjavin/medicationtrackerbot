...
The cost is printed on the tin. {{product}} has no reset flow, no security questions, and no email loop. The only paper in this system is the recovery code from day one: 160 random bits. The key comes out of the chip in your hand, and my server only ever meets the locked result.
