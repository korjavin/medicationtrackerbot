...
The list has three shapes: some of it waits for my yes, some runs the moment I use the feature, and some I can replace with a provider of my own. Flattening those into one comforting "all opt-in" sentence would read better, and it would be false. {{product}} says which is which, next to each feature.
