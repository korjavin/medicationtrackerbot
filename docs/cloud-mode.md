...
- **Account deletion is a verified local wipe** (med-d5t.8 / med-yor.3): after `DELETE /api/account` succeeds, the client's `clearLocalVault()` awaits `deleteDatabase` on **both** `medtracker-cloud` (encrypted mirror + LDK material) and Dexie's `MedTrackerDB` (the shared frontend's plaintext caches), and only navigates away once both are confirmed gone — a blocked delete (another open tab) surfaces a recoverable "close other tabs" error instead of a false success, and a retry skips the (now-impossible) re-auth ceremony and re-runs only the wipe. Push unsubscribe (browser-side only — the server rows are already cascaded) and SW unregister are best-effort. The pre-delete safety export warns before writing plaintext provider keys, same as Settings → Import/Export. Related invariant: all `medtracker-cloud` opens go through `localdb.js`'s `openDb()`, which auto-closes on `versionchange` so a live handle in the same tab never blocks the deletion (or a future schema bump); any new direct open of that DB must do the same.
