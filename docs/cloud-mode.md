...
| **AI with your own OpenAI(-compatible) key** — *not a carve-out* | Meal and activity descriptions, meal photos, Telegram assistant messages and their tool results, Journey narration summaries | Your configured endpoint, browser-direct. Operator sees: nothing — never touches the operator | Whatever your provider does; the operator stores nothing | **No toggle — active on use** — whenever you have a key set and use an AI feature | This IS the BYO path | `web/cloud/js/aiclient.js:17`, `web/cloud/js/aiclient.js:295` |
...
| **Telegram relay** | Outbound reminder and confirmation text the client composes, forwarded verbatim; your chat id | The operator's relay, then api.telegram.org. Operator sees: plaintext in transit | The queued entry is deleted after sending; Telegram keeps the chat | **Opt-in** — requires you to link your own bot | The bot is yours; a chat bot cannot be made end-to-end encrypted | `internal/cloudserver/push.go:215`, `internal/cloudserver/telegram.go:1`, `internal/cloudserver/inbox.go:207` |
...
**Dry-queue safety net**: if the browser stops re-uploading the horizon, reminders stop — and the server can't extend them, because it can't compute a schedule. So an hourly sweep (`Relay.StaleSyncSweep` → `Repo.AccountsNeedingStaleSyncWarning`) sends the fixed, content-free warning push "Open the app to keep reminders running", at most once per 24h per account. This is the E2EE analogue of the adherence safety net.

The predicate is **horizon exhaustion, not user absence** (bd med-2lx). An account qualifies when the latest *unsent* `scheduled_pushes` entry fires within `CLOUD_DRY_QUEUE_WARN_HOURS` (default 120h) — and an account with *no* unsent entries at all collapses to zero, i.e. always qualifies, because that is the state where reminders have already stopped. It is deliberately **not** gated on `last_sync_unix`: every inbox drain flushes ops through the sync API and touches that column, so an account alive enough to tap Telegram Confirm buttons looked permanently "freshly synced" while its horizon rotted.

Two guards keep the empty-queue case from becoming a nag for everyone who never wanted reminders: the account must have at least one **enabled** push subscription (the warning is itself a web push, so this is also what makes it deliverable at all) and must have a scheduled push, in any state, that fired **within the last `CLOUD_DRY_QUEUE_WARN_HOURS`** — `MarkPushSent` blanks a fired row's payload but never deletes it, so a queue that ran for months and then went dry still qualifies while an account that never armed reminders never does.

That recency bound also makes the warning **escalate-then-stop**. An account that switched every reminder off uploads an empty replace-all schedule, which is byte for byte the same server-side state as an account whose browser stopped re-uploading; telling them apart would mean remembering when the client last PUT a schedule. Instead the window bounds every class: a real dry queue is warned from the first sweep after its last reminder fired (daily, so the failure surfaces on day one), and an account still empty a window later — deliberately off, or long abandoned — goes permanently quiet rather than being nagged forever.
...
- **Record types**: `medication` (server field names incl. `rxcui`/`normalized_name`/`tz_shift_policy`), `intake` (deterministic id `intake-<medId>-<slotUnix>` for scheduled doses — the multi-device dedup mechanism; random id for manual log-past), `restock` (one per restock event), `tzplan` (one record per proposed/active shift — steps live inside the record, no pre-materialization into intake rows), `medreminderpref` (singleton, reminder enable/disable), `slotmeds` (singleton, `slotUnix → [medicationId…]` — which meds each pushed reminder NAMED, so a Telegram Confirm resolves the tap by identity rather than by a time band; written after every successful `pushSchedule`, merge-and-prune with a 48h retention, and in the vault rather than a device store because the device that pushes is routinely not the device that drains the tap — bd med-eas.65).
...
- **Reminders = compute-and-upload**: `computeReminderHorizon` (in `reminders.js`) walks a 7-day window one day at a time (working around `PlanDoses`'s "today + tomorrow" forecast cap without touching `medschedule.js`), producing `{fire_at_unix, text}` entries; the shim debounce-recomputes (2s) and PUTs the replace-all schedule via the existing C0c `pushSchedule` after intake/med/tzplan mutations and on unlock. The **inbox-drain path does not use that debounce** (bd med-9y9): `cloud-boot.js`'s `afterApply` awaits `recomputeAndPush` directly, because most drains are triggered by the SW's `inbox-wake` push into a backgrounded tab the browser may freeze or discard well inside 2s — and because a long ack burst (a `.nxk` import acking ~100 sealed events) reset the timer on every applied drain, starving the recompute entirely.
