...
| **Push reminder relay** — *not a carve-out* | The time a reminder fires, and an opaque blob; the payload is app-layer encrypted under the push key on top of RFC 8291 | The operator's queue, then your browser vendor's push service (FCM / APNs / Mozilla). Operator sees: ciphertext only | The queue entry is deleted once sent; the subscription endpoint persists until you disable push | **Opt-in** — you grant notification permission | n/a | `internal/cloudserver/push.go:215`, `internal/cloudserver/relay.go:88` |
