...
- **Conflict resolution is client-side** (server can't merge what it can't read): last-writer-wins per record on `clientTs`.
  ...
  3. **A floor for materialized rows** (bd med-d4w, bd med-9a87). A row that a *read path* lazily derives into a **deterministic recordId** is stamped `clientTs: 0`, never `correctedNow`. Such a row is re-derivable on every device, so a client whose mirror is stale re-creates that same recordId in its *initial* state — `PENDING` dose, `pending` session. Stamped with a real timestamp that stale re-creation is the newest write, and LWW erases the real state underneath it. A floor makes materialization lose the merge against a real write, which is exactly its standing: **it only ever needs to win against nothing at all.** Guard 2 does not cover this — it only fires when the writing device can already *see* the record, and the whole failure mode is a device that cannot.
     ...
     **The floor is not absolute through the local write path** (bd med-qhpu). `writeRecord` applies guard 2 unconditionally, and `getRecord` reads the *raw* store — tombstones included — so a floored row is promoted to `existing.clientTs + 1` whenever any raw row already occupies that recordId: after a `nextVariant` delete (where re-materialization *should* stick, which is why this cannot be fixed by special-casing `0`), or if a real row lands between the read path's absence check and its `put`. It holds for the case that matters — a device with no local row at all, which is what med-9a87 was — but the guarantee wants an atomic put-if-absent on the records port to be unconditional. Current materializers: `web/domain/medintake.js` (dose targets) and `web/domain/workout.js`'s `getNext` (the day's session). See [Incident: a stale tab erased a finished workout](#incident-a-stale-tab-erased-a-finished-workout-bd-med-9a87).
...
**The rule that prevents it** is guard 3 above: materialized rows are stamped `clientTs: 0`. Regression coverage lives in `web/static/js/tests/cloud.shim-contract.workout-convergence.test.js`, which replays a stale device against a completed session using a local replica of `applyIncoming`'s comparison — it exercises the domain layer, not the sync port, so it does not cover the skew/promotion behavior above.

**Known gaps, filed rather than fixed here**: the floor is not atomic (bd med-qhpu); scheduled sessions still mint a *non-deterministic* numeric id, so two devices can hold divergent bodies at equal `clientTs` until the first real write, orphaning logs written against the losing one (bd med-8j12); and the same stale-device shape reaches read/timer-side read-modify-writes on singleton records in `gamification.js` and `tzplan.js`, which the med-9a87 sweep wrongly waved through as "last-write-wins is intended" (bd med-y4ue).
