...
| **Telegram relay** | Outbound reminder and confirmation text the client composes, forwarded verbatim; the opaque medication ids a dose reminder names, stored on its queued row; your chat id | The operator's relay, then api.telegram.org. Operator sees: plaintext in transit | The queued entry is deleted after sending; Telegram keeps the chat | **Opt-in** — requires you to link your own bot | The bot is yours; a chat bot cannot be made end-to-end encrypted | `internal/cloudserver/push.go:215`, `internal/cloudserver/push.go:229`, `internal/cloudserver/telegram.go:1`, `internal/cloudserver/inbox.go:207` |
