...
- **Conflict resolution is client-side** (server can't merge what it can't read): last-writer-wins per record on `clientTs`.
  ...
  3. **A floor for materialized rows** (bd med-d4w, bd med-9a87). A row that a *read path* lazily derives into a **deterministic recordId** is stamped `clientTs: 0`, never `correctedNow`. Such a row is re-derivable on every device, so a client whose mirror is stale re-creates that same recordId in its *initial* state — `PENDING` dose, `pending` session. Stamped with a real timestamp that stale re-creation is the newest write, and LWW erases the real state underneath it. A floor makes materialization lose the merge against a real write, which is exactly its standing: **it only ever needs to win against nothing at all.** Guard 2 does not cover this — it only fires when the writing device can already *see* the record, and the whole failure mode is a device that cannot.
     ...
     **The floor also covers read/timer-side *transitions*** (bd med-y4ue). A record that already exists everywhere is not materialized, but a read or timer that flips its state is the same hazard one layer up: it rebuilds the record out of a mirror that may be stale, so `now()` lets it push state it never observed over newer state it never saw. Here guard 2's promotion is the feature, not the leak — a floored write becomes `existing.clientTs + 1`, i.e. **it beats exactly the version this device read, and nothing newer**; when it loses, the transition is simply re-derived from the winning record on the next read. Current floored transitions: `gamification.js` (`writeJournal({...}, { derived: true })` for `resolveElapsedChapter`/`getTraits`/`appendKeystone`, and `resolveElapsed`'s frozen verdict) and `tzplan.js`'s `refreshPlanStatus`. Deliberate user actions keep `now()` — editing on a stale device is ordinary LWW. Residual: two devices deriving *different* fields of the same singleton land on equal stamps and diverge until the next real write merges them (the blob rebuild converges them; nothing is erased).
...
**Known gaps, filed rather than fixed here**: the floor is not atomic (bd med-qhpu); scheduled sessions still mint a *non-deterministic* numeric id, so two devices can hold divergent bodies at equal `clientTs` until the first real write, orphaning logs written against the losing one (bd med-8j12); and the same stale-device shape reached read/timer-side read-modify-writes on singleton records in `gamification.js` and `tzplan.js`, which the med-9a87 sweep wrongly waved through as "last-write-wins is intended" — since fixed by extending the floor to those transitions (bd med-y4ue).
