...
| **Voice with your own ElevenLabs key** — *not a carve-out* | Voice audio, transcripts, the tool definitions this app provisions, and the tool RESULTS — the blood pressure, weight and diary notes the agent reads out of your vault | api.elevenlabs.io, browser-direct. Operator sees: nothing — never touches the operator | ElevenLabs retains the conversation per your ElevenLabs account settings; the provisioned agent and tool definitions persist in that account until deleted | **No toggle — active on use** — whenever you start a voice call with your own key set | This IS the BYO path | `web/cloud/js/elevenlabs-agent.js:12`, `web/cloud/js/elevenlabs-agent.js:13`, `web/cloud/js/elevenlabs-signed-url.js:8` |
