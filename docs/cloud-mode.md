...
- **CSP**: the account-app CSP (`internal/cloudserver/router.go` `setSecurityHeaders`) relaxes `script-src`/`worker-src`/`media-src` for account subdomains to load the `@elevenlabs/client` SDK locally and run its blob: AudioWorklets (mirroring bot mode). The voice SDK has been vendored to eliminate the zero-third-party-script defense weakening on the DEK-bearing page.
