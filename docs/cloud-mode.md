...
- **Response.** `{items:[{id, kind, app_version, created_at, ciphertext_b64}]}`, **newest-first** (`ListRecentFeedback`), capped at 100. The direction is load-bearing: the per-account cap does not bound the queue *globally*, so past 100 undrained rows `feedbackpull`'s oldest-first drain order would push the very item the DM announced off the end of the window — and the page has no pagination and, until med-rbl.3, no way to drain. **No `account_id`, no `client_id`** — web feedback is anonymous by design and this endpoint must not be the thing that de-anonymizes it. Missing / unknown / expired tokens all get an identical 401 body.
