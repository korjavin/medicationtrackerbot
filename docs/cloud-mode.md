...
| **Trial voice (operator's ElevenLabs key)** | Your voice audio, the transcript, and the agent conversation — including tool results read out of your vault | The operator mints the signed URL, then the operator's ElevenLabs account. Operator sees: plaintext in transit | Not stored by the app; ElevenLabs retention applies | **Opt-in + explicit consent (revocable in Settings → Integrations)** — the `voice` consent scope, separate again | With your own ElevenLabs key the operator is not involved | `internal/cloudserver/trial_proxy.go:100`, `internal/cloudserver/trial_proxy.go:54` |
...
| **Voice with your own ElevenLabs key** — *not a carve-out* | Voice audio, transcripts, the tool definitions this app provisions, and the tool RESULTS — the blood pressure, weight, workout sessions and diary notes the agent reads out of your vault | api.elevenlabs.io, browser-direct. Operator sees: nothing — never touches the operator | ElevenLabs retains the conversation per your ElevenLabs account settings; the provisioned agent and tool definitions persist in that account until deleted | **No toggle — active on use** — whenever you start a voice call with your own key set | This IS the BYO path | `web/cloud/js/elevenlabs-agent.js:12`, `web/cloud/js/elevenlabs-agent.js:13`, `web/cloud/js/elevenlabs-signed-url.js:8` |
...
- **Concrete tools dispatch in-tab, no relay.** At `startSession`, cloud mode registers `clientTools` whose names match the provisioned tools; each callback dispatches into `window.CloudMCPDispatcher.handle(...)` (the `createDispatcher({ router })` catalog the Claude connector uses) — e.g. `log_blood_pressure({systolic,diastolic,pulse})` → `bp.create {measured_at:<now ISO>, ...}`, `get_blood_pressure({days})` → `bp.list`. Writes stamp `measured_at`/timestamps = now client-side to match the catalog op schemas. `get_workout` is the one callback that chains two ops — `workouts.sessions.next` for the session, then `workouts.sessions.details` for the per-exercise log rows — because `log_exercise` needs a log id and a voice LLM asked to chain two reads before a write mostly won't. The generic `mcp_help`/`mcp_call` callbacks stay registered, but they are **not** in `TOOL_SPECS`, so the BYO agent is never told they exist and can never invoke them; they remain only because the trial agent's tool list lives in the operator's ElevenLabs dashboard, outside this repo. No relay hop, no crypto — the tab is both the voice client and the MCP responder host, online + unlocked during its own call. Bot mode passes no `clientTools` (unchanged); nothing goes to the cloud server.
...
- **Scope**: the voice agent's reach is exactly `TOOL_SPECS` — 9 flat-param tools (bp/weight/notes read+write, plus the med-eas.82 workout trio). It has **no** generic `mcp_call`: that callback exists in the browser but is not provisioned, so the agent cannot reach the rest of the catalog (the Claude connector, which speaks MCP directly, still can). Adding a domain to voice therefore means adding concrete tools + a prompt line + a `TOOLSET_VERSION` bump, not widening a bridge. Still un-voiced: food, meds, sleep. On the BYO path a bump reprovisions silently on the next call; the **trial** agent (`TRIAL_ELEVENLABS_AGENT_ID`) is configured in the ElevenLabs dashboard, so its tool list must be updated there by hand to match. Server-initiated / off-device voice is the separate med-65c relay path.
