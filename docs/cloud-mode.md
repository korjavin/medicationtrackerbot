...
- **With C2d done, C2e (full-vault export/import, both modes) is the only unported C2 piece** — the post-C2d unmapped-route warn list on the rig is its final input (every remaining unmapped route should belong to export/import machinery, not a live UI call site). Plan: `docs/plans/2026-07-06-cloud-c2e-vault-export-import.md`.

## Voice (ElevenLabs)

Cloud mode runs the ElevenLabs conversational agent **browser-direct**, with dynamic MCP tools dispatched in-tab — the viability spike for the cloud voice pillar (bd med-rgc). Plan: `docs/plans/2026-07-06-cloud-voice-elevenlabs-mcp-poc.md`.

- **Signed URL is minted in the browser, not the server.** Bot mode hits `GET /api/elevenlabs/signed-url` (server-side, to hide the operator key); cloud has no such route, which is why "Call Agent" 404'd. `web/cloud/js/elevenlabs-signed-url.js` `createElevenLabsClient({ settingsDomain })` reads the unmasked `elevenlabs.{api_key, agent_id}` from the vault (`settingsDomain.readIntegrationsUnmasked()`, the C2c BYO pattern) and calls `GET https://api.elevenlabs.io/v1/convai/conversation/get_signed_url?agent_id=<id>` with the `xi-api-key` header directly. Published as `window.CloudElevenLabs` from `apishim.js`; `fetchSignedURL()` in `web/static/js/features/elevenlabs-call.js` short-circuits to it when `window.__MEDTRACKER_CLOUD__`. **The BYO ElevenLabs key never crosses `/api`** — it goes browser→ElevenLabs only.
- **Dynamic MCP tools dispatch in-tab, no relay.** At `startSession`, cloud mode registers `mcp_help` + `mcp_call` as ElevenLabs SDK `clientTools`; their callbacks call `window.CloudMCPDispatcher.handle(...)` (the same `createDispatcher({ bp, weight, notes })` catalog the Claude connector uses, published from `apishim.js`) straight in-process and return JSON strings the agent reads. No relay hop, no crypto — the tab is both the voice client and the MCP responder host, and it's online + unlocked during its own call. Bot mode passes no `clientTools` (unchanged). MCP dispatch stays in-tab; nothing goes to the cloud server.
- **Manual prerequisites (the PoC acceptance).** The user's ElevenLabs **agent dashboard** must declare two client tools — `mcp_help` (no params) and `mcp_call` (params: `op` string, `params` object) — both marked **blocking** so the agent awaits the result, with a system prompt telling it to call `mcp_help` first to discover ops then `mcp_call` to fetch data. And the vault `elevenlabs.{api_key, agent_id}` must be set in Settings → Integrations. Leakage note: ElevenLabs' cloud sees tool names, results, and transcripts (inherent to any cloud voice agent); under BYO keys that's strictly user↔ElevenLabs — the zero-knowledge server sees nothing.
