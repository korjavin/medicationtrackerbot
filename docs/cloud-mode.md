...
**Settings entry point**: the main app's Settings screen (`web/static`, not just the `/unlock` shell) surfaces a cloud-only Notifications block (`features/settings.js`, gated on `window.__MEDTRACKER_CLOUD__`) with Enable/Disable and "Send test push" controls. It calls the same DOM-free `web/cloud/js/push.js` (`subscribe`/`unsubscribe`) and `web/cloud/js/reminders.js` (`sendTestPush`) helpers the shell uses, reached via `window.MedTrackerCloud.ctx` (published by `cloud-boot.js` once the vault is unlocked) — no new backend route. `sendTestPush` appends one test entry to the real computed reminder set and PUTs both together in a single replace-all call, so it never clobbers the real schedule. The `/unlock` shell's own push screen remains for the pre-app flow (before the vault has ever been unlocked in the main app).
