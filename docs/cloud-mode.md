...
- Seal only the Telegram **`file_id`** (plus `mime`/`size`) into the mailbox as a `tg_photo` event (`sealPhoto` in `internal/cloudserver/telegram.go` — the largest rendition's `file_id`, structurally identical to `sealCommand`). Never seal the bytes: multi-MB blobs in `inbox_events` would bloat every `GET /api/inbox` drain, and the mailbox is a control channel, not a blob store.
