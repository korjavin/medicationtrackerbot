...
- **Response.** `{items:[{id, kind, app_version, created_at, ciphertext_b64}]}`, oldest-first, capped at 100. **No `account_id`, no `client_id`** — web feedback is anonymous by design and this endpoint must not be the thing that de-anonymizes it. Missing / unknown / expired tokens all get an identical 401 body.
