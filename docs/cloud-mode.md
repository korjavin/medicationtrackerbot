...
Every row below is an implemented data flow. **They do not share one activation
story, and flattening them into "all opt-in" is the regression this table exists
to prevent.** Three activation classes for the rows that leave the device:
...
3. **Always on, inherent to running the service** — operational metadata,
   including the plaintext record-type tag on every synced op.

Two further row kinds appear in the table: paths marked *not a carve-out*, which
are named so their strong wording is auditable rather than assumed, and one
marked *stays on your device*, which never leaves the device but is readable by
anyone holding it. The `How it is turned on` column states each row's class.
...
the metadata-only signals are also in
[Metadata leakage summary](#metadata-leakage-summary).

**This table is generated.** Its source is the privacy/egress manifest at
`web/cloud/js/privacy-manifest.js`, which is also what
`web/cloud/js/privacy.js` renders into Settings → *What can the operator see?*.
Edit the manifest and run `pnpm privacy:docs`; editing the table by hand fails
CI. `web/cloud/js/tests/architecture.privacy-claims.test.js` additionally
derives the set of things that MUST be disclosed from the code — every
outbound-HTTP / server-side-plaintext file under `internal/cloudserver/` and
every literal third-party host in the Go and browser sources — so a new proxy,
upstream host, or plaintext path fails CI until a manifest entry claims it.

<!-- BEGIN GENERATED privacy-boundary-table — source: web/cloud/js/privacy-manifest.js, regenerate: pnpm privacy:docs -->

| Boundary | What leaves the vault | Who sees it | Retention | How it is turned on | If you bring your own key | Evidence |
|---|---|---|---|---|---|---|
| **The vault (all health records)** — *the vault promise* | Medications and doses, blood pressure, weight, food, workouts, sleep, vitals, diary notes | The operator's sync API — as ciphertext only. Operator sees: ciphertext only | Stored until you delete the account; the operator holds no key to it, ever | **Always on** — the default and only storage path | n/a — the DEK is generated in your browser and never leaves your devices | `web/cloud/js/crypto.js:1`, `internal/cloudserver/sync.go:1` |
| **Record types and record ids (sync tags)** | Which KIND of record each sync op is (`bp`, `medication`, `hrsample`, `diary`, …) and its record id — never its contents | Stored beside the ciphertext in the operator's oplog. Operator sees: metadata only | For the life of the op or snapshot, same as the ciphertext it labels | **Always on** — the tag is what lets a reading device bind the AAD without a schema change | n/a | `web/cloud/js/sync.js:54`, `web/cloud/js/sync.js:59`, `web/cloud/js/sync.js:1193` |
| **Decrypted mirror on your device + warm unlock** — *stays on your device* | Every record you have synced, decrypted, in this browser's IndexedDB — plus a cached device key that reopens the vault with no passkey prompt | Nowhere. It never leaves the device. Operator sees: nothing — never touches the operator | Until you sign out, delete the account, or clear site data on that browser; there is no idle auto-lock | **On-device, inherent to offline use** — the app renders offline, which requires a local plaintext copy | n/a | `web/cloud/js/sync.js:211`, `web/cloud/js/unlock.js:19`, `web/cloud/js/unlock.js:38` |
| **Push reminder relay** — *not a carve-out* | The time a reminder fires — plus, if you linked Telegram, the time a message you send the bot arrives, which fires a content-free wake — and an opaque blob; a reminder payload is app-layer encrypted under the push key on top of RFC 8291 | The operator's queue, then your browser vendor's push service (FCM / APNs / Mozilla). Operator sees: ciphertext only | The queue entry is deleted once sent; the subscription endpoint persists until you disable push | **Opt-in** — you grant notification permission | n/a | `internal/cloudserver/push.go:215`, `internal/cloudserver/relay.go:88`, `internal/cloudserver/relay.go:384` |
| **Account existence + subdomain** | That an account exists, its subdomain, and its creation date | The operator; DNS/SNI observers on the network. Operator sees: metadata only | For the life of the account | **Always on** — a per-user origin is how the app is served | n/a | `internal/cloudserver/router.go:201` |
| **Sync cadence, blob sizes, IPs** | How often each device syncs, how big each encrypted blob is, and the IP it came from | The operator. Operator sees: metadata only | Row timestamps persist with the oplog; request logs per the operator's retention policy | **Always on** — syncing is how your devices agree | n/a | `internal/cloudserver/sync.go:1` |
| **Trial AI (operator's OpenAI key)** | The meal or activity description you typed, and meal PHOTOS, in plaintext | The operator's server, then the operator's OpenAI(-compatible) account. Operator sees: plaintext in transit | Not stored by the app; the provider's own retention applies (cloud-operations-security.md §5) | **Opt-in + explicit consent (revocable in Settings → Integrations)** — the `ai` consent scope; skipping key setup is not consent | Add your own OpenAI key and the request goes browser-direct, never through the operator | `internal/cloudserver/trial_proxy.go:99`, `web/cloud/js/aiclient.js:320`, `web/domain/settings.js:56` |
| **Trial AI assistant (Telegram free text + tool results)** | The Telegram message you sent, AND the vault data the assistant's tools read to answer it (BP history, notes, medications, …), carried back into the conversation as tool results | The operator's server, then the operator's OpenAI(-compatible) account. Operator sees: plaintext in transit | Not stored by the app; provider retention applies | **Opt-in + explicit consent (revocable in Settings → Integrations)** — the `tg` consent scope, separate from meal parsing | With your own OpenAI key the whole loop runs browser-direct | `web/cloud/js/tg-agent.js:120`, `web/cloud/js/tg-agent.js:137`, `web/cloud/js/inbox-apply.js:778`, `web/cloud/js/aiclient.js:434` |
| **Journey narration summaries** | Already-computed gamification summaries — streaks, weekly counts, in-range percentages, chapter and experiment state. Never raw records | Your own OpenAI(-compatible) endpoint, or — with no key of your own — the operator's server and the operator's OpenAI account. Operator sees: plaintext in transit | Not stored by the app; provider retention applies | **Opt-in + explicit consent (revocable in Settings → Integrations)** — shares the `tg` consent scope with the Telegram assistant; refusal degrades to the deterministic card | With your own OpenAI key the summary goes browser-direct | `web/cloud/js/gamification-narrator.js:141`, `web/cloud/js/apishim.js:150`, `web/cloud/js/aiclient.js:434` |
| **Trial voice (operator's ElevenLabs key)** | Your voice audio, the transcript, and the agent conversation — including tool results read out of your vault, which can be any part of your health record | The operator mints the signed URL, then the operator's ElevenLabs account. Operator sees: plaintext in transit | Not stored by the app; ElevenLabs retention applies | **Opt-in + explicit consent (revocable in Settings → Integrations)** — the `voice` consent scope, separate again | With your own ElevenLabs key the operator is not involved | `internal/cloudserver/trial_proxy.go:100`, `internal/cloudserver/trial_proxy.go:54` |
| **AI with your own OpenAI(-compatible) key** — *not a carve-out* | Meal and activity descriptions, meal photos, Telegram assistant messages and their tool results, Journey narration summaries, plus a health-data-free model-list request when you press "Load models" in Settings | Your configured endpoint, browser-direct. Operator sees: nothing — never touches the operator | Whatever your provider does; the operator stores nothing | **No toggle — active on use** — whenever you have a key set and use an AI feature, or press "Load models" in Settings | This IS the BYO path | `web/cloud/js/aiclient.js:17`, `web/cloud/js/aiclient.js:374`, `web/cloud/js/aiclient.js:139` |
| **Voice with your own ElevenLabs key** — *not a carve-out* | Voice audio, transcripts, the tool definitions this app provisions, and the tool RESULTS — the agent can call any operation in the app catalog, so any part of your health record it is asked about (blood pressure, weight, workouts, medications, food, sleep, diary notes) is read out of your vault into the conversation | api.elevenlabs.io, browser-direct. Operator sees: nothing — never touches the operator | ElevenLabs retains the conversation per your ElevenLabs account settings; the provisioned agent and tool definitions persist in that account until deleted | **No toggle — active on use** — whenever you start a voice call with your own key set | This IS the BYO path | `web/cloud/js/elevenlabs-agent.js:12`, `web/cloud/js/elevenlabs-agent.js:13`, `web/cloud/js/elevenlabs-signed-url.js:8` |
| **Food search with your own food-DB endpoint** — *not a carve-out* | Search terms and scanned barcodes | Your configured food-DB endpoint, browser-direct. Operator sees: nothing — never touches the operator | Whatever your endpoint does | **No toggle — active on use** — whenever you search food with your own endpoint set | This IS the BYO path — setting an endpoint is what removes the operator | `web/cloud/js/fooddb.js:84`, `web/cloud/js/fooddb.js:88` |
| **Telegram relay** | Outbound reminder and confirmation text the client composes, forwarded verbatim; the opaque medication ids a dose reminder names, stored on its queued row; your chat id | The operator's relay, then api.telegram.org. Operator sees: plaintext in transit | Reminder text is erased from the queued entry when it is sent; the medication ids and dose-slot key stay on the sent row for 48 hours so a later Confirm tap still resolves, then are erased; Telegram keeps the chat | **Opt-in** — requires you to link your own bot | The bot is yours; a chat bot cannot be made end-to-end encrypted | `internal/cloudserver/push.go:215`, `internal/cloudserver/push.go:229`, `internal/cloudserver/telegram.go:1`, `internal/cloudserver/inbox.go:207` |
| **Inbound Telegram message, before sealing** | The text (or file reference) of a message you send the bot, in server memory for the instant it takes to seal it | The operator's relay process. Operator sees: plaintext, transiently in memory | Never stored unsealed; the sealed copy sits in your inbox until an unlocked device drains it | **Opt-in** — only if you linked Telegram | n/a — Telegram delivers bot updates in the clear | `internal/cloudserver/inbox.go:207` |
| **Telegram bot token at rest** | Your bot token, sealed under a key DERIVED FROM `SESSION_SECRET` — a server holding that secret can recover it. This is not vault-grade | The operator's database. Operator sees: plaintext at rest (the operator holds the unwrapping key) | Until you unlink the bot | **Opt-in** — stored when you link Telegram | n/a | `internal/cloudserver/tg_token.go:22`, `internal/cloudserver/tg_token.go:55` |
| **Hosted MCP (tier 2)** | Full MCP query AND response content, in plaintext in server memory — the operator runs the shim | The operator, then the hosted AI client (claude.ai / ChatGPT). Operator sees: plaintext in transit | Content not stored; the pairing key persists while remote mode is enabled | **Opt-in** — explicit enable, deleted on Disconnect | Tier 1 (local shim) gives the same capability over a blind relay | `internal/cloudserver/mcp_remote.go:253`, `internal/cloudserver/mcp_endpoint.go:1` |
| **Local MCP (tier 1)** — *not a carve-out* | Nothing. Frames are opaque; the relay sees sizes, timing and pairing ids only | A blind WebSocket pipe to your own machine. Operator sees: metadata only | No content retained | **Opt-in** — pairing with a local shim | n/a | `internal/cloudserver/mcp_relay.go:15` |
| **Hosted MCP pairing key at rest** | The pairing key, stored server-side while hosted mode is enabled | The operator's database. Operator sees: plaintext at rest (the operator holds the unwrapping key) | Deleted on Disconnect; the token itself is never logged | **Opt-in** — only with hosted mode | n/a | `internal/cloudserver/mcp_remote.go:34` |
| **Operator-default food DB** | Food search terms and scanned barcodes, through a same-origin operator proxy — not browser-direct | The operator, then the operator's food-DB instance. Operator sees: plaintext in transit | Not stored by the app; mind the operator's HTTP access logs — the term travels in the query string | **No toggle — active on use** — whenever you search food without a food-DB endpoint of your own | Set your own endpoint in Settings → Integrations and the operator leaves the path entirely | `internal/cloudserver/food_proxy.go:48`, `internal/cloudserver/food_proxy.go:49` |
| **Drug lookups (RxNav)** | Drug-name and interaction queries, through a blind same-origin proxy | The operator in transit, then RxNav (NIH). Operator sees: plaintext in transit | Never logged or stored by the application; only the resolved rxcui lands on the medication record. Mind the reverse proxy's access logs — the name travels in the query string | **No toggle — active on use** — whenever a medication lookup or interaction check runs | None — there is no BYO alternative for drug lookups | `internal/cloudserver/rxnav_proxy.go:56`, `internal/cloudserver/rxnav_proxy.go:59` |
| **Mi Band (.nxk) backup import** | The whole backup — heart rate, SpO2, stress, sleep, steps, workout history — written to a temporary file on the operator's disk and parsed in plaintext before it is sealed | The operator's server process and its temp directory. Nothing is sent onward. Operator sees: plaintext, transiently in memory | The temp file is deleted when the request finishes; only sealed events are stored. Refused outright when no unlocked device has published an inbox key | **Only on your action** — when you upload a backup, in the app or to the Telegram bot | n/a — the parser needs the raw file | `internal/cloudserver/vitals_import_api.go:42`, `internal/cloudserver/vitals_import_api.go:86`, `internal/cloudserver/telegram.go:1137` |
| **In-app feedback** — *not a carve-out* | What you type, plus any screenshot or voice note you attach — age-encrypted in your browser to the developer's public key | The operator stores the ciphertext blindly; the developer decrypts it offline. Operator sees: ciphertext only | Queued until the developer drains it; the app version and feedback kind travel as plaintext metadata | **Only on your action** — when you send feedback from inside the app | n/a | `web/cloud/js/feedback-submit.js:10`, `internal/cloudserver/feedback.go:39` |
| **Feedback sent to the Telegram bot** | Your message text and any voice note or screenshot, in plaintext on the operator's server; the original message is then copied verbatim into the developer's Telegram admin chat | The operator's server (plaintext), Telegram, and the developer's admin chat. The queued copy is encrypted server-side to the developer's key. Operator sees: plaintext in transit | The admin-chat copy stays in that Telegram chat; the queued ciphertext waits until the developer drains it | **Only on your action** — when you tap "Send feedback" in the manager bot and send the next message | n/a — use the in-app feedback form for the browser-encrypted path | `internal/cloudserver/feedback_telegram.go:168`, `internal/cloudserver/feedback_telegram.go:185`, `internal/cloudserver/feedback_telegram.go:280` |

<!-- END GENERATED privacy-boundary-table -->
...
| Meal photo pixels, at rest — only the Telegram `file_id` (plus the caption, sealed like any inbound text) is stored | That an inbound message arrived, its size, and its timestamp |
...
4. **Install + notifications, platform-aware** — detect iOS/Android and show matching add-to-homescreen steps. Ordering is load-bearing on iOS: **install first, then enable push** — web push only works inside the installed app and the permission prompt needs a user gesture there. Shown only when it can change the outcome (bd `med-eas.63`): a **mobile** browser that is not already `display-mode: standalone`. Desktop is never nudged — it delivers push to a plain tab — and the copy is platform-honest in both directions: iOS is told a Safari tab gets *no* push at all, Android is told push works either way and install is a convenience. There is **no auto-redirect into the installed app**: iOS has no API for it and Android's `beforeinstallprompt` only prompts an install, so guiding ("install, then reopen from your Home Screen and unlock with the same passkey") is the whole of what the platform allows. The escape hatch — "Continue in browser anyway" — never blocks.
...
- **Conflict resolution is client-side** (server can't merge what it can't read): last-writer-wins per record on `clientTs`.
  ...
  3. **A floor for materialized rows** (bd med-d4w, bd med-9a87). A row that a *read path* lazily derives into a **deterministic recordId** is stamped `clientTs: 0`, never `correctedNow`. Such a row is re-derivable on every device, so a client whose mirror is stale re-creates that same recordId in its *initial* state — `PENDING` dose, `pending` session. Stamped with a real timestamp that stale re-creation is the newest write, and LWW erases the real state underneath it. A floor makes materialization lose the merge against a real write, which is exactly its standing: **it only ever needs to win against nothing at all.** Guard 2 does not cover this — it only fires when the writing device can already *see* the record, and the whole failure mode is a device that cannot.

     **The floor rides on an atomic put-if-absent** (bd med-qhpu), because a floor alone is not enough. `writeRecord` applies guard 2 unconditionally and `getRecord` reads the *raw* store, tombstones included, so a floored row used to be promoted to `existing.clientTs + 1` whenever any raw row already occupied that recordId — the exact promotion the floor exists to prevent. `recordsPort.putIfAbsent` (`writeRecord`'s `ifAbsent` mode) is the derived-write primitive instead: under `withRecordsLock` it writes only into an **empty raw slot**, treats a tombstone as occupied, stamps the floored `clientTs` verbatim (no skew correction — derived bodies must stay byte-identical across devices, bd med-8j12), and resolves to the **stored winner** rather than overwriting it. `applyIncoming` takes the same lock, so an incoming op cannot land between the absence check and the put. A caller that loses decides what that means: `getNext` drops the day and surfaces the next occurrence.

     A slot the *derived* path finds tombstoned therefore stays empty, which makes **who tombstones a slot** load-bearing. For workout sessions only a deliberate `deleteSession` does — `nextVariant` is now an explicit **replacement** of the slot (cascade the logs, advance the rotation, write the new variant as a real wall-clock write) rather than a delete that leant on the next `getNext` re-materializing over the tombstone. Any new derived write must ask the same question: *is there a delete path whose recovery depends on re-materialization?* If yes, make that path write what it wants instead of deleting it.

     A session tombstone therefore means "no workout that day" to **every** reader, not just `getNext` (bd med-w0fe):
     - `findOrCreateScheduledSession` probes the raw slot (`listRaw`, a READ — the body it mints is a real wall-clock write and must keep guard 1's skew correction, which `putIfAbsent`'s verbatim stamp would drop) and returns null on a tombstone: a Telegram Snooze/Skip tap on the already-sent reminder no-ops instead of minting a fresh body that guard 2 would promote above the tombstone, and `startSession`'s med-gmyf re-key mints an ad-hoc session (same as for a day already finished).
     - The reminder horizon reads the **raw** session rows (`recordsPort.listRaw`, the one reader that needs them) and treats a tombstone at `session-<groupId>-<date>` as that day suppressed. Before, `sessionStatusByKey` saw only live rows, so a deleted day read as "not materialized yet": the card skipped it while Telegram kept asking about it, and tapping Snooze/Skip re-materialized the slot.

     Deleting is therefore a **one-way door for that day**, including a day already completed and deleted from history: nothing re-materializes the slot, so the card, the horizon and the Telegram buttons all stay silent for it until the next occurrence. A vault export drops tombstones (`web/domain/vault.js`), so an export/import round trip does bring the day back.

     Materializers: `web/domain/workout.js`'s `getNext` (the day's session) uses `putIfAbsent`. `web/domain/medintake.js` (dose targets) still uses `put` + the floor; its own slot guard means promotion is only reachable over a `deleteFutureIntakes` tombstone — where it also resurrects the deleted dose — so it wants the same treatment (not yet filed as its own bead). See [Incident: a stale tab erased a finished workout](#incident-a-stale-tab-erased-a-finished-workout-bd-med-9a87).
...
**The rule that prevents it** is guard 3 above: materialized rows are stamped `clientTs: 0` **and go in via `putIfAbsent`**, so they can neither outrank a real write nor be promoted over one. Regression coverage lives in `web/static/js/tests/cloud.shim-contract.workout-convergence.test.js` (domain layer, replaying a stale device against a completed session through a local replica of `applyIncoming`'s comparison) and in `web/cloud/js/tests/sync.test.js` (the port itself: put-if-absent over a tombstone, over a live row, and the promotion that used to happen).

**Since fixed**: scheduled sessions used to mint a *non-deterministic* numeric id, so two devices held divergent bodies at equal `clientTs` until the first real write and logs written against the losing one orphaned — `derivedNumericId(recordId)` now hashes the slot instead (bd med-8j12).

**Known gaps, filed rather than fixed here**: the same stale-device shape reaches read/timer-side read-modify-writes on singleton records in `gamification.js` and `tzplan.js`, which the med-9a87 sweep wrongly waved through as "last-write-wins is intended" (bd med-y4ue).
...
1. Client (which has the domain logic + data) computes all reminders for the next horizon (default 30 days, configurable; ~KBs even at many-meds scale) and uploads `(fire_at, app_ciphertext)` rows, replace-all per sync — replace-all pre-schedule semantics.
...
**Dry-queue safety net**: if the user doesn't open the app within the horizon, reminders stop — and the server can't extend them. The server can still *see the queue emptying* (inherent metadata: `scheduled_pushes.fire_at_unix`), so it sends a generic warning push ("Open the app to keep reminders running") once a day per account. This is the E2EE analogue of the adherence safety net.

The signal is **queue exhaustion, not sync recency** (bd med-2lx, `Repo.AccountsNeedingStaleSyncWarning`). Two facts made the original last-sync gate a net that never caught anything: every inbox drain flushes ops through the sync API and touches `sync_state.last_sync_unix`, so an account alive enough to tap Confirm in Telegram always looks freshly synced while its horizon rots; and the query INNER JOINed a subquery over *unsent* rows, so an account whose queue had already gone fully dry produced no row at all and could never be warned. A production account missed five days of morning doses through both. The query now takes `COALESCE(MAX(fire_at_unix), 0)` from a correlated subquery — an empty queue collapses to 0 and always trips the warn horizon — and, because that would otherwise sweep in every account that never wanted reminders, it warns only accounts with a live (non-disabled) push subscription **and** a most-recent scheduled push (sent or not) no further than `dryQueueWithin` *behind* now. That backward window is deliberate: a user who switches reminders off leaves the same trace as a horizon that rotted (`ReplaceSchedule` deletes the unsent rows, the sent history stays), and no query outside the vault can tell those apart — so the warning is bounded to a few daily nags after the last reminder rather than made permanent.
...
- **Drain protocol (binding requirements for C3b).** The failure to design out: an event acked as processed that never landed in the vault. Rules:
  ...
  4. **Apply in sealed-timestamp order** within a drain, and backdate records from the sealed `at_unix` (not drain time) — that's what makes the 09:00 tap record 09:00. See rule 4 under the implementation notes for what `at_unix` means per event kind.
- **Drain wedge recovery (`med-eas.51`).** The ack-after-flush barrier has a failure mode: an event whose ops can never flush is never acked, so the drain re-fetches the whole un-acked backlog on every 5s poll — forever. The concrete trigger seen in the wild is a stale sealed Telegram `.nxk` **Mi Band vitals import** (`vitals_import`, `sealNXKDocument` in `internal/cloudserver/telegram.go`, applied by `web/cloud/js/inbox-apply.js` → `web/domain/vitals.js importSamples`). `importSamples` day-batches hr/spo2/stress into **one record per day holding all that day's samples**, so a densely-sampling device day serializes past `maxOpCTLen = 64<<10` (64 KiB) — `POST /api/sync/ops` then returns `400 "op field too large or missing"` (`PostOps`, `internal/cloudserver/sync.go`). Three consecutive permanent 4xx trip `WRITE_ERROR_BUDGET` and set `syncWedged`; `flushConfirmed` returns false immediately, the event never acks, and the un-byte-capped `GET /api/inbox` re-fetches the entire ~160 MB backlog every poll. That is a self-inflicted DoS that bricks the account. Three coupled guards stop it:
  1. **Pause the drain when wedged.** `drainInbox` checks `isSyncWedged()` (`web/cloud/js/sync.js`) *before* fetching and returns without a single `GET /api/inbox`. `startInboxPolling` backs off the visible-tab poll on consecutive no-progress ticks and resumes on progress. Clearing `syncWedged` (via reset) un-pauses the next tick — no new persisted flag. **One drain outcome is deliberately excluded from that backoff (`med-3q8.3`):** an event this device cannot *open* — sealed to an inbox public key a concurrent device's last-write-wins `PUT /api/inbox/key` superseded. It is reported per event (`unopenable` in the drain report), left queued so a device holding the matching private key can still drain it, and stepped over rather than thrown — a throw used to abort the whole page, so every Telegram command behind it stayed `⏳ Queued` for as long as the app stayed open. Because such an event is skipped client-side after a fetch guard 2 already byte-caps, it costs exactly what an empty poll costs and must not throttle the mailbox.
  2. **Byte-cap the page.** `ListInbox` (`internal/cloudserver/inbox.go`) trims each response to `maxInboxDrainBytes = 1<<20` (≈1 MiB, mirroring sync's `FLUSH_MAX_BODY_BYTES`) alongside the existing `maxInboxDrainBatch=200` count cap, always including at least the first event so a single over-budget event still makes progress. So one response is never 160 MB.
  3. **Reset clears the poison backlog.** `DELETE /api/inbox` (session-scoped, `ClearInbox` → `cloudstore.ClearInboxEvents`) drops every un-acked event for the account. `resetLocalSync` (`web/cloud/js/cloud-boot.js`) calls it best-effort after the local un-wedge, so the one "Reset local sync" action also discards the un-appliable sealed events. **This discards any un-applied sealed events** — same "discards un-synced local writes" semantics reset already carries; it is the recovery escape hatch, and acceptable as such.
  The deeper fix — not sealing a large Mi Band `.nxk` as one giant >64 KiB event into the per-message mailbox (chunk the vitals import into sub-`maxOpCTLen` records, or route bulk imports around the per-message inbox) — is a **follow-up** (bd med-0cf), deliberately out of scope for the loop fix.
...
**Photos — proxy, never store. Implemented (bd med-vcv.1).** A photo is worse: the relay must download it, because only the relay holds the bot token. The rule:

- Seal only the Telegram **`file_id`** (plus `mime`/`size`, and the **caption** verbatim — unparsed, like any inbound text; it is the only place a portion the pixels cannot show ("300g") can come from, and the client passes it to the vision model as a hint) into the mailbox as a `tg_photo` event (`sealPhoto` in `internal/cloudserver/telegram.go` — the largest rendition's `file_id`, structurally identical to `sealCommand`). Never seal the bytes: multi-MB blobs in `inbox_events` would bloat every `GET /api/inbox` drain, and the mailbox is a control channel, not a blob store.
- On drain, the client requests the image through a **session-gated, account-scoped relay endpoint** (`GET /api/telegram/photo?file_id=…`, `GetPhoto`) that resolves the `file_id` via `getFile` — through **this account's own bot token**, resolved from the session via `BotByAccount` — and **streams the bytes through** to the browser. Nothing is written to disk; nothing is logged but the status code. The vision parse then runs browser-side (`applyTGPhoto` → `foodAI.parseMealFromPhoto`) with the user's own or the trial key. **The account-scoping IS the whole boundary:** the relay cannot cross-check the requested `file_id` against the sealed mailbox (it is zero-knowledge ciphertext), but a `getFile` with bot A's token can only ever resolve files sent to bot A, so a session can only fetch its own bot's images.
- This is durable, contrary to first appearances: a Telegram **`file_id` is stable and re-usable** — it is the `file_path` returned by `getFile` that expires (~1h). The relay re-resolves on demand at drain time, so a photo sent on Monday and drained on Friday still fetches, as long as the file exists on Telegram's servers and the bot token is unchanged. A `getFile`/download failure is surfaced to the client as a normal error and the event is acked, not retried forever (a re-drain would re-pull bytes and re-bill the provider for a result that will not improve). Idempotent on success via per-item `tg-<mailboxEventId>-<index>` ids, like `/food`.
...
**Latency.** Nothing in cloud mode polled — no SSE, no change stream — so a drain only happened on page load. `startInboxPolling` now drains every 5s **whether or not the tab is visible**, and immediately on `visibilitychange`. Hidden tabs poll too (med-d15): the user is in Telegram exactly while this tab is backgrounded, so skipping the timer there made the mailbox silent for the whole window that matters — the ✅ only ever landed the instant they switched back. Browsers already clamp background timers to roughly one fire per minute, so the cost is about one empty `GET /api/inbox` per minute per backgrounded tab. With the app open, "Queued" becomes "Recorded" in a few seconds; backgrounded, within about a minute; with it closed, on next unlock.

**The wake (`med-5fo`).** The poll is the floor, not the only signal. A hidden tab's timer is throttled to ~1/min and a frozen page does not poll at all, but the server knows the *instant* a seal lands — so every seal path calls `TelegramAPI.sealAndQueue`, which fires `Relay.WakeInbox` (`internal/cloudserver/relay.go`). That fans a content-free `{"kind":"inbox-wake"}` out to the account's push subscriptions on the same non-NK channel as the stale-sync warning, coalesced to **at most one push per account per 10s** (a drain is per-account, not per-message, so the tab woken by the first message of a burst drains the rest). `web/cloud/sw.js` postMessages every open window — which `cloud-boot.js` turns into an immediate `drainInbox`, bypassing the poller's backoff gate — and suppresses the notification only when a window **acknowledges** the nudge on the transferred `MessageChannel` port within 1.5s. Existence is deliberately not the signal: `clients.matchAll` also returns a window that is frozen, still loading, or *locked*, and the ack listener is installed in cloud-boot's post-unlock path, so a locked window never handles the wake — exactly the case where the notification is the only thing that gets the event recorded. The page acks on receipt, not after its drain, so a slow drain cannot earn a duplicate notification. With no ack (or no window) the SW shows a FIXED client-side string, never server-supplied text. A failed seal wakes nothing.

**Known tax.** The subscription is `userVisibleOnly: true` (no browser offers silent push otherwise), so Chrome tolerates only a bounded number of notification-less pushes before it forces its own generic "this site was updated in the background" one. On a phone, a chatty Telegram day can therefore surface a notification per burst once that budget is spent. The wake is additive: no notification permission (or no subscription) means no wake, and the poll still applies.
...
- **Web feedback → metadata ping only.** `FeedbackAPI` calls an optional `notify(kind, appVersion)` hook (`TelegramAPI.NotifyFeedback`) that DMs `📮 New feedback (web) · kind … · app … · <time> — read it here (link expires in 30 min): https://<base>/feedback#t=<token>`. **No content, no ciphertext, no account id** — the server holds a blob it cannot read and the E2EE promise stays literally true. The client-side encryption (`web/cloud/js/feedback-submit.js`) and the queue are untouched; reading still requires the age private key.
...
**Never on the request path.** Both relays run on their own goroutine with a detached, timeout-bounded context — the web ping fires with `go a.notify(...)` *after* the 204 (whose request context dies with the response), and the Telegram relay is detached from the webhook so two more Bot API calls can't push it toward a redelivery (a redelivered feedback message finds the armed flag already cleared and would get a confusing onboarding reply). Every failure — a 403 because the admin never pressed `/start`, a refused `copyMessage` that falls back to sending the text — is a `slog.Warn`, never an error to the user: a broken notification must not turn a stored feedback item into a 500.

**One queued item, one DM.** `AppendFeedback` returns `queued bool` and the web ping only fires when it inserted, so the reliable-retry client re-POSTing the same `client_id` over a flaky connection doesn't announce "new feedback" again. `FEEDBACK_ADMIN_CHAT_ID` unset (or unparseable — logged and treated as unset) means no relay at all.

### Web feedback reader: `/feedback` (med-rbl)

The metadata ping told the developer *when* to run `feedbackpull` — a chore needing the server's SQLite file, the age private key (which must not be on that server), and a Go toolchain, so in practice nobody ran it. The reader page moves the read to the phone the DM already arrived on, **without moving the key to the server**.

- **Page.** `web/cloud/feedback.html` + `web/cloud/js/feedback-reader.js`, served on the **base domain** (`router.go`'s base-domain branch returns before the subdomain ceremony-page switch, so both halves are handled there). Web feedback is anonymous — there is no account to scope it to. It is a plain shell page: `connect-src 'self'`, no app-document egress widening, and the only `web/static` asset the base domain serves is the vendored `age.min.js` the page decrypts with (an exact-path case, not a `/static/` prefix — copying the 150 KB bundle into `web/cloud/vendor` would be a second copy to drift).
- **Capability, not an account.** `cmd/cloud` has no admin HTTP surface at all (admin ops are a CLI subcommand), so rather than invent admin accounts the short-lived token **is** the capability, delivered over an already-authenticated channel: the developer's own Telegram DM. `feedback_reader_tokens` (migration 021) stores **SHA-256 of the token only**, with a 30-minute TTL, expired rows swept on the next mint (no background job — mirrors `SweepExpiredClaims`). It is **multi-use within the TTL**: the developer reloads the page, and a one-shot token would look like a bug. Validation walks the unexpired rows with `subtle.ConstantTimeCompare` rather than `WHERE token_hash = ?`.
- **Fragment, then header.** The DM link puts the token in the **URL fragment** (`/feedback#t=…`). Browsers never send a fragment to the server, so it stays out of access logs — and Telegram *prefetches* links to build previews, which would fetch a query-param token before the developer ever tapped it. The page reads it out of the fragment, immediately `history.replaceState`s it away, and sends it in the **`X-Feedback-Reader-Token` header** on `GET /api/feedback/queue`. A query-param token is not accepted.
- **Response.** `{items:[{id, kind, app_version, created_at, ciphertext_b64}]}`, **newest-first** (`ListRecentFeedback`), capped at 100. The direction is load-bearing: the per-account cap does not bound the queue *globally*, so past 100 undrained rows `feedbackpull`'s oldest-first drain order would push the very item the DM announced off the end of the window — and the page had no pagination and, before med-rbl.3, no way to drain. **No `account_id`, no `client_id`** — web feedback is anonymous by design and this endpoint must not be the thing that de-anonymizes it. Missing / unknown / expired tokens all get an identical 401 body.
- **Decryption is in the browser.** The pasted age identity goes into the vendored typage `Decrypter` (`addIdentity`, the sibling of the `addPassphrase` path `web/static/js/core/backup-crypto.js` already uses for `.json.age` import). Per-item failure is isolated — one unreadable row renders an inline error and the rest still show, mirroring `cmd/feedbackpull`'s fail-open drain. A malformed key is detected once, before the loop.
- **Ack: delete after reading (med-rbl.3).** `DELETE /api/feedback/queue` with `{"ids":[…]}` deletes exactly those rows and answers `{"acked":n}` — a count, never the rows, so the delete path cannot become the thing that attributes anonymous feedback. It is the **same capability token in the same header** as the read (one `authorized()` gate serves both verbs, so they cannot drift), and the id list is mandatory: an empty list, a missing field, or more than 100 ids is a 400. There is deliberately **no "drain everything" and no "delete older than"** — those are exactly the shapes that would ack rows nobody has looked at. Acking an id that is already gone is a 200 (`DeleteFeedback` is a no-op), so a second tab or a retried tap is not an error. A body on `DELETE` rather than a query string: the ids are the selector and a query string would print them into every access log for nothing.
- **Ordering rule: rendered, *then* acked.** `cmd/feedbackpull`'s `run()` renders each item into a buffer and writes it **before** deleting, so a failed write cannot destroy the only copy of the plaintext. The page holds the same property, and it *has* to hold it client-side: the server never sees plaintext, so it cannot verify anything was read. `renderResults` attaches the per-item **Delete** control only inside the success branch — an item that failed to decrypt gets no control at all, so "never delete what the developer has not seen" is a property of the markup rather than a rule to remember. The batch **"Delete all N read"** button carries only the ids that decrypted; the unreadable ones stay on screen *and* in the queue for `feedbackpull`. A rejected ack (expired token, network) removes nothing and shows a fixed message.
- **Key handling (required mitigation).** The owner accepted the "paste a private key into a web page" risk *against these mitigations*, so they are part of the deliverable: the key is never written to localStorage / sessionStorage / IndexedDB / a cookie / the URL, never logged, and never interpolated into an error message (every message the module produces is a fixed constant, so no library exception text can carry key material). The field is cleared the moment its value is read and again on `pagehide`. JS strings are immutable, so dropping references is the ceiling — the bytes cannot be zeroed. If web feedback ever becomes high-volume, the upgrade is wrapping the age identity in a WebAuthn-PRF envelope (machinery already in `web/cloud/js/crypto.js`) so nothing is typed at all.
- **Degradation.** A token-mint failure logs a `slog.Warn` and the ping falls back to the old "run feedbackpull to read" text — it never drops the ping and never touches the feedback POST. A deployment that never calls `SetFeedbackReader` 404s the queue endpoint. `cmd/feedbackpull` stays the fallback reader (and remains the only place account attribution is visible).
...
**Trial consent** (bd med-yor.2): every trial request is additionally gated client-side by an explicit, durable consent record — a `trialconsent` encrypted-vault singleton (`web/domain/settings.js`, routed at `PATCH/GET /api/settings/trial-consent` in the apishim) with three independent boolean-or-null scopes: `ai` (meal text/photo parsing), `voice` (trial ElevenLabs calls), and `tg` (the Telegram free-text agent and gamification narrator, whose messages carry vault-derived health data). Only literal `true` passes; `null`/missing means never asked and refuses — skipping key setup in the wizard is NOT consent. The disclosure dialog (`web/static/js/features/trial-consent.js`) is shown before first trial use and names the data categories, that content transits the operator's provider account, and the BYO alternative; consent is grantable and revocable in Settings → Integrations. BYO keys bypass the gate entirely. The record syncs across devices like any vault record but is deliberately excluded from export/import backups — a fresh account asks fresh consent. The server never sees consent state; the trial proxy stays dumb.
...
- **Write ops require `mode: 'write'` plus a non-empty `intent`.** Any catalog op with
  ...
  `add_note`, `log_exercise`, `set_workout_status`) send `mode: "write"` plus an intent, and its
  generic `mcp_call` forwards whatever `mode`/`intent` the agent stated rather than stripping
  them.
...
- **Token-in-path access-log caveat** (same shape as `initData` in [docs/sse-traefik.md](archive/sse-traefik.md#initdata-exposure-in-traefik-access-logs)): the token travels in the URL path (`/mcp/<token>`), so Traefik's default access log writes it to disk on every request. The leak is bounded by the same throttle that bounds guessing it, but treat Traefik logs as sensitive for this route and consider the same mitigations (drop query/path logging for `/mcp/*`, or a redaction filter) if that's a concern for your deployment.
...
| Sync cadence, blob sizes, IPs | cloud | standard for any sync service; no record *contents*. Not "no signal", though: cadence, reminder times, inbox arrivals and blob sizes are **sensitive metadata with real inference risk** — together they sketch a routine and how much of what a user tracks. Treat every metadata row here that way, not as categorically harmless |
| TG bot token, chat id, TG message text (both directions) in transit | cloud + Telegram | opt-in; **a bot channel cannot be end-to-end encrypted, so text crosses the relay in plaintext both ways**. Outbound reminder text defaults to `detailed` (names the medication); Settings → Notifications → *Telegram Reminder Detail* switches it to `generic` ("Medication time", no names). Only entries with `delivery` of `telegram`/`both` carry outbound text; `ct` stays opaque. Inbound messages transit the relay in the clear too, but the **relay** **seals each on arrival and never parses it, never calls AI on it, and never logs it** — only the unlocked app opens and acts on them (the bot replies "Queued", then fills in once the app records it). Note the scope of that claim: it is about the relay, not about what happens next. The unlocked tab may hand the text to an AI assistant, and on the operator's trial key that means the message *and* the vault data the assistant's tools read transit the operator again (`web/cloud/js/tg-agent.js`, consent scope `tg`). Photos are fetched through the server but never stored |
...
| Record types + record ids (sync tags) | cloud | inherent to the wire format: `record_type:recordId` rides beside every op in the clear so a reading device can bind the AAD (`web/cloud/js/sync.js:54-60`). **This is a health-inference channel, not neutral plumbing** — a record-type histogram plus arrival times says a lot (a BP-heavy, twice-daily profile reads as hypertension monitoring), and the vitals record ids embed the calendar day (`hrsample-2026-07-08`). No mitigation beyond padding/batching, which is not implemented |
| Voice audio, transcripts + tool results (ElevenLabs) | the user's own ElevenLabs account (BYO), or the operator + the operator's ElevenLabs account (trial) | BYO goes browser-direct; the tool RESULTS the agent reads back (BP, weight, notes) travel with the conversation either way. The trial path additionally crosses the operator and needs the `voice` consent scope |
| Mi Band .nxk backup contents (transient, server-side) | cloud, for the duration of the upload | the ONE place the server opens health data: the `.nxk` is written to a temp file and parsed server-side before being sealed to the inbox (`internal/cloudserver/vitals_import_api.go:86`, Telegram path `telegram.go:1137`). Temp file removed when the request ends; refused outright with no published inbox key, so nothing plaintext is ever stored |
...
- **Cloud boot path**: `cloud-boot.js` runs before `app.js`, sets `window.__MEDTRACKER_CLOUD__`, warm-unlocks via the LDK cache, installs the shim, and kicks `pullOnOpen`. `app.js`'s `checkAuth()` short-circuits on that flag (CLAUDE.md rule 11) — the Telegram login screen never renders in cloud mode. No change stream (SSE/poller) in cloud mode; repaint is optimistic-write + pull-then-`invalidateTags`.
...
- **Vitals ingestion (med-nzz)**: C2a shipped the read side only; a full-vault import (C2e) is one way records arrive. Direct Mi Band ingestion now also exists via **NXK upload + Telegram `.nxk`** — the server parses the `.nxk` backup with the existing Go parsers (`internal/domain/nxk`), seals the parsed HR/SpO2/sleep/stress/daystats/mi-band-workout streams to the account's sealed inbox, and the client drains and writes them into vault vitals records (idempotent by natural key, exactly-once via the drain's ack-after-flush barrier). **No live device webhook and no GPS** — the original per-account Mi Notify webhook is dropped; `ParseOutdoorWorkouts`'s GPS return is discarded. Entry points: session-gated `POST /api/vitals/import` (Settings → Import/Export upload control) and the cloud Telegram bot's `.nxk` document branch. Same trust model as the inbound plaintext policy — the server sees plaintext only transiently, then seals it. See `docs/plans/20260711-cloud-miband-nxk-ingest.md`.
...
- **Record types**: `medication` (server field names incl. `rxcui`/`normalized_name`/`tz_shift_policy`), `intake` (deterministic id `intake-<medId>-<slotUnix>` for scheduled doses — the multi-device dedup mechanism; random id for manual log-past), `restock` (one per restock event), `tzplan` (one record per proposed/active shift — steps live inside the record, no pre-materialization into intake rows), `medreminderpref` (singleton, reminder enable/disable). There is no slot→meds record any more: the identity a Telegram Confirm resolves by rides on the relay's own reminder row (`scheduled_pushes.tg_med_ids`) and is sealed into the tap event — bd med-kbpf retired the `slotmeds` records, whose every reconstruction failure meant hourly nags for doses already taken. Stray tombstones from that record type are simply an unknown type nothing reads.
...
- **RxNorm same-origin-proxied** (`web/cloud/js/rxnorm.js` → `internal/cloudserver/rxnav_proxy.go`): the RxNav lookup + interaction calls route through the operator's blind same-origin proxy (`/api/rxnav/*`) — the DEK page's `connect-src` is `'self'` + BYO hosts only, so browser-direct fetches to the RxNav hosts were structurally CSP-blocked. The proxy is blind by the fixed-string log invariant; see the metadata-leakage table above. The interaction-list endpoint has been decommissioned by NLM (403s on every call); `checkInteractions()` degrades to `[]` on any non-OK response, so med save still succeeds without an interaction warning.
...
- **Route table** (`web/cloud/js/apishim.js`): the query-param CRUD style (`/create`, `/update`, `/delete`, `?id=`) is preserved verbatim rather than folded into a combined GET+POST base path like bp/weight/food. `rotation/state`, `rotation/initialize`, `exercises/unique` and `sessions/schedule` have no frontend caller but are catalogued MCP ops, so med-csu.3 routed them too. Still intentionally unmapped, falling through to the unmapped-route warning (a code comment in the shim lists them so the warn stays interpretable): the legacy `session/snooze`/`session/skip` compat routes. (The external Mi Band Notify webhook is likewise not an apishim route, but mi-band ingestion is no longer unmapped — it now arrives via the server-side NXK parse→seal path, med-nzz; see "Vitals ingestion" above.)
...
**Already covered by #613** (batched flushes + permanent-vs-transient classification + bounded compaction + shared import-UX busy/beforeunload guard) stopped the *tight retry loop* and the over-cap 400 storm, but a permanently-failing write batch still sat in `pending` and was re-POSTed on **every open, forever** — no retry budget, no manual un-wedge. This PR adds both:

- **Write-error retry budget → `syncWedged` pause.** `sync.js` counts consecutive permanent (413/400, via `isPermanentSyncStatus`) write-error opens in a device-local `sync_meta.writeErrorStreak`; at `WRITE_ERROR_BUDGET = 3` it sets `sync_meta.syncWedged`. Once wedged, `flushPending` early-returns before the drain loop — writes still queue durably to `pending` (nothing is lost), the doomed batch just stops being re-posted. Only permanent 4xx increments the streak; transient failures (offline/5xx/429) never touch it, so a flaky network can't wedge a device. Any accepted batch resets the streak to 0. Both keys default-safe when absent (streak 0, not wedged), so older devices upgrade cleanly. `getSyncStatus()` surfaces `wedged`, and `describeSyncStatus()` adds a plain-text recovery hint when set.
- **"Reset local sync (rebuild from server)" affordance.** `resetLocalSync(ctx)` (`sync.js`, wrapped as `window.CloudVault.resetLocalSync` in `cloud-boot.js`, exposed in Settings → Import/Export as a cloud-only control) clears the `records`, `pending`, and `sync_meta` stores in **one atomic IDB transaction** under `withRecordsLock`, then `pullOnOpen` re-bootstraps this device from the server's compacted snapshot + tail. It **discards un-synced local pending writes by design** — the un-wedge escape hatch — and does **not** touch the `device` store (NK/LDK/crypto survive). Clearing `sync_meta` nulls `localLastSeq`/`syncWedged`/`writeError`/`forceSnapshotPending`, so the next bootstrap starts clean; a crash mid-reset leaves a null cursor the next bootstrap heals. It also cheaply escapes a bloated-oplog-every-open device (one snapshot pull, not a full oplog re-download). The Settings button confirms, runs under the shared `importInFlight`/`setImportBusy` re-entry guard so it can't collide with an in-flight import, then reloads.

**med-0ol.8 finding — no fix needed.** Full-vault (archive/json/gzip) `CloudVault.importAll` lands via `replaceAllRecords` (local, zero ops) + `forceSnapshot` → `tryForceSnapshot`, which posts exactly **1 bump op + 1 gzip'd snapshot** — a **constant 2 requests regardless of vault size**, not per-record ops, and it already carries the same shared `setImportBusy` UX safeguards as the `.nxk` import. A snapshot-path invariant test in `web/cloud/js/tests/sync.test.js` (1500 records → exactly one `/api/sync/ops` POST + one `/api/sync/snapshot` POST) pins this so a future regression to per-op writes fails CI.
...
- **Provisioned from code, key is enough.** `web/cloud/js/elevenlabs-agent.js` `createElevenLabsAgentProvisioner({ settingsDomain })` reads the vault `elevenlabs.api_key` and calls the ElevenLabs Agents/Tools API browser-direct (CORS-open). `provision()` orchestrates `ensureTools()` → `ensureAgent()`: `ensureTools()` lists `GET /v1/convai/tools`, matches our fixed spec list by name, and `POST /v1/convai/tools` (the `{tool_config:{type:"client",...}}` shape) for any missing → `{name→id}` map; `ensureAgent(toolIds)` `POST /v1/convai/agents/create` with `tool_ids`, a strong system prompt (always call a tool for any bp/weight/workout/notes question, never claim no access), `tts.voice_id`, and `tool_call_sound:'typing'` + `tool_call_sound_behavior:'always'` for the audible tool-call cue. **Idempotent by `TOOLSET_VERSION`**: the provisioned `{agentId, toolsetVersion, toolIds}` is persisted to a dedicated `voiceprovisioning` vault singleton (`settingsDomain.get/setVoiceProvisioning`) and reused on a matching version — reprovision only on first run or a version bump, never per call. If the user pre-set `elevenlabs.agent_id`, that agent is `PATCH`ed instead of creating one. Published as `window.CloudElevenLabsAgent` from `apishim.js`. The provisioned tools are: `get_blood_pressure`(days?)/`log_blood_pressure`(systolic,diastolic,pulse?), `get_weight`/`log_weight`(kg), `get_notes`/`add_note`(text,tag?), and — since bd med-eas.82 — `get_workout`()/`log_exercise`(log_id?,exercise_id?,sets?,reps?,weight_kg?,notes?,status?)/`set_workout_status`(session_id,status), plus the generic pair `mcp_help`(query?,topic?,operation_id?) and `mcp_call`(op,params_json?,mode?,intent?,path_params_json?). `mcp_help`'s `operation_id` drill-down is what gives the agent an op's exact schema, so it is declared, not just the no-arg catalog dump. Flat typed params throughout — ElevenLabs client tools take no nested objects, which is why `mcp_call`'s `params`/`path_params` travel as JSON **strings** the callback parses.
...
- **Concrete tools dispatch in-tab, no relay.** At `startSession`, cloud mode registers `clientTools` whose names match the provisioned tools; each callback dispatches into `window.CloudMCPDispatcher.handle(...)` (the `createDispatcher({ router })` catalog the Claude connector uses) — e.g. `log_blood_pressure({systolic,diastolic,pulse})` → `bp.create {measured_at:<now ISO>, ...}`, `get_blood_pressure({days})` → `bp.list`. Writes stamp `measured_at`/timestamps = now client-side to match the catalog op schemas. `get_workout` is the one callback that chains three ops — `workouts.sessions.next` for the session, `workouts.sessions.details` for the persisted log rows, then `workouts.exercises.list` for the planned exercises that have no log row yet (`sessions.details` returns only persisted logs, and an untouched session has none; the Workouts screen synthesizes the same missing rows). The plan comes from the session's `exercise_snapshot` when it has one (so an exercise removed from today only stays removed) and from the live variant otherwise, and only schedule-sourced logs consume a planned row — a mid-session `source:"library"` log's `exercise_id` indexes a different table. It flattens them into one `exercises` array where each row carries `log_id` (0 when un-logged) + `exercise_id`, and `log_exercise` picks `logs.update` or `logs.create` off that — otherwise an update against an unknown id succeeds with an empty body and the agent reports a write that never happened. `log_exercise` re-reads before it writes, which is what lets it refuse a session `sessions.next` surfaced for a LATER day (logging into one would record the workout on the wrong date and propagate the numbers into that day's plan) and fall back to the planned targets when the agent says only “skip the rows”. `set_workout_status('in_progress')` routes to `workouts.sessions.start`, not `sessions.status`, so `started_at`/snooze/day-re-keying stay correct, and answers with the re-read workout because a start moves the ids; a terminal transition answers with the written status instead, since `sessions.next` excludes terminal sessions and would hand back an unrelated future workout. `mcp_help`/`mcp_call` are provisioned too (see Scope) — `mcp_call` accepts `params_json`/`path_params_json` strings and `JSON.parse`s them, since ElevenLabs client tools cannot declare nested objects. No relay hop, no crypto — the tab is both the voice client and the MCP responder host, online + unlocked during its own call. Bot mode passes no `clientTools` (unchanged); nothing goes to the cloud server.
...
- **Scope — catalog parity, hybrid surface (bd med-eas.82)**: the voice agent reaches **every** operation the Claude connector does, because `mcp_help` + `mcp_call` are now provisioned in `TOOL_SPECS`. They had existed as `clientTools` callbacks since med-csu.3 but were never in the spec list, so the agent was never told they existed — which is why "modify my workout" ended up as a Vitals note (`add_note` was the only free-text write it had). The nine concrete tools stay alongside them: med-eas.26 found voice LLMs drive concrete flat-param tools far more reliably, so bp/weight/notes/workouts keep theirs and the generic pair covers the long tail (food, meds, sleep, vitals, stats, settings). Adding a domain to voice is therefore optional now — a concrete tool is a reliability upgrade, not the only way in. On the BYO path a `TOOLSET_VERSION` bump reprovisions silently on the next call; the **trial** agent (`TRIAL_ELEVENLABS_AGENT_ID`) lives in the operator's ElevenLabs account and no server code touches the management API, so the operator must re-run `pnpm trial:agent --apply` (`scripts/provision-trial-agent.mjs`, bd med-qgnk) after each bump — it pushes the same `TOOL_SPECS` and agent config to the shared agent, so the two paths no longer drift by hand, but it is a deliberate operator step, not automatic. Server-initiated / off-device voice is the separate med-65c relay path.
...
- **C3b — Telegram delivery + inbound** (after C0c):
  ...
  - **Inbound (implemented, med-76c.2)** — inline Confirm/Snooze buttons plus the sealed inbound mailbox, in two parts. Part 1 (PR #511) shipped the `mt/v1/inbox` sealed box, `accounts.inbox_public_key` + `inbox_events`, the `PUT/GET/DELETE /api/inbox*` endpoints and the `SealAndQueue` write seam. Part 2 added `tgclient` `CallbackQuery`/`answerCallbackQuery`/inline keyboards, the `callback_query` branch in `ChildWebhook`, a per-entry `tg_callback` stem on the queue (migration `013_push_tg_callback.sql`), and the client drain. The drain protocol is implemented as specified above:
    ...
    4. **Sealed-timestamp order** — events are sorted by the sealed `at_unix` (not mailbox arrival order) and records are backdated to it, so a Confirm tapped at 09:00 records `taken_at` 09:00 even when the app first opens at noon. What `at_unix` *is* depends on the kind (bd med-3hr): a **callback-query tap** (`intake_slot_action`, `workout_session_action`, `measure_reminder_action`) carries the relay's clock at tap time — the message it hangs off is the bot's own reminder, so it has no useful date of its own — while an inbound **message** (`tg_command`, `tg_photo`, `tg_text`) carries the message's own Telegram `date` (`tgclient.Message.AtUnix`), falling back to the relay clock only when Telegram omits it or it is more than 5 minutes in the future. Old dates are never clamped: a webhook retried after relay downtime must still record a `/bp` at the minute it was sent, not at the minute the relay came back.
    5. **Identity on the row, cancel at the tap** (med-kbpf) — the schedule PUT carries `tg_med_ids` (the medication record ids the message names) beside `tg_text`; the relay stores it, copies it down the re-fire chain, and seals it into the tap event. A Confirm therefore **cancels the slot's re-fire chain server-side at tap time**, the drain confirms exactly the named meds (deterministic id first, that med's `minDoseInterval` band as the drift fallback), and only re-arms one re-fire (`POST /api/telegram/rearm-refire`) if a named med is still due after it applies. An in-app confirm still cancels via `POST /api/telegram/cancel-refire` — which since med-r3dm accepts every reminder stem (`s:`/`w:`/`bp:`/`wt:`, validated by `tgclient.ValidCallbackStem`) and also **deletes the message still live in the chat**, so an in-app workout complete/skip/start no longer leaves its reminder sitting there with buttons (migration `023_push_tg_message_id.sql` records the sent message id; the in-app workout call sites are wired in `apishim.js`, and since med-9bmb so are the BP/weight write routes, which rebuild today's `bp:`/`wt:` slot stem from the reminder pref). The sent row keeps `tg_callback`/`tg_med_ids`/`tg_message_id` (`MarkPushSent` scrubs only `ct`/`tg_text`) and the relay's hourly sweep expires them 48h later (`ScrubSentPushIdentity`, the same window the retired `slotmeds` records used), so a tap on last night's message still resolves this morning. A tap carrying no ids (pre-med-kbpf reminder, or one older than that window) applies nothing and answers the message rather than guessing from a time band. Privacy: the relay learns opaque numeric ids per slot — strictly less than the medication NAMES `tg_text` already hands it at `detailed` verbosity. Migration `022_push_med_ids.sql`.
