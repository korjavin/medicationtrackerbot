...
- **With C2d done, C2e (server-mode migration exporter + import) is the only unported C2 piece** — the post-C2d unmapped-route warn list on the rig is its final input (every remaining unmapped route belongs to `cmd/exporter`/import machinery, not a live UI call site).
...
- **C2 — remaining domains**: medications + intake log + reminder computation, food (incl. direct-from-browser AI/vision/barcode), workouts, vitals, sleep, diary, tz handling. Closes with the **server-mode migration pair**: `cmd/exporter` (full-vault JSON) + cloud-client import landing as one encrypted snapshot — see "Migrating an existing server-mode install".
