...
| GET | `/api/gamification/rings` | Slim Today-widget projection: `{enabled, level, today_hp, rings:[{ring, hp, closed}]}` — per-ring today HP plus `closed` (true when the ring earned a non-floor outcome/consistency award today, not just the honesty floor). Drives the Today "X of 5 rings closed" line + "your move" picker. (MCP op `gamification.rings`.) |
