...
| GET | `/api/settings` | User settings bundle. Returns the same shape `/api/bootstrap` embeds under `settings`, so the frontend's `loadSettings()` SWR fetcher can refresh toggles, food targets, reminder status, timezone, weight-unit preference, and (when set) tab order in a single round-trip. Response: `{timezone, server_time, server_timezone, weight_unit_preference, features, food_targets, bp_reminder_status, weight_reminder_status, tab_order?}`. `features` is the same map as `/api/init` / bootstrap (`food, bp, weight, medication, workout, health`). `bp_reminder_status` / `weight_reminder_status` are `null` when the request has no authenticated user. `tab_order` is omitted (not `null`) when unset so clients preserve their local fallback. |
