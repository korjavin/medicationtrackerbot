...
| GET | `/api/gamification/rings` | Slim Today-widget projection: `{enabled, level, today_hp, rings:[{ring, hp, closed, sync_pending}]}` — per-ring today HP plus `closed` (true when the ring earned a non-floor outcome/consistency award today, not just the honesty floor) and `sync_pending` (see `/api/gamification/summary`). Drives the Today "X of 5 rings closed" line + "your move" picker, which skips `sync_pending` rings. (MCP op `gamification.rings`.) |
