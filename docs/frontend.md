...
- **Sub-tab groups inside section views stay** (`.med-tabs`, `.workout-tabs`, `.wg-food-subtabs`, `.health-tabs`): use `bindTabGroup()` / `activateTabGroup()`. Health sub-tabs are "Overview" (charts) and "Notes" (diary); Notes loads lazily. Active sub-tab persists to `localStorage` under `mt-<section>-subtab` (e.g. `mt-food-subtab` with values `log` / `meals` / `fooddb`); the BP range selector follows the same `mt-bp-range` convention.
