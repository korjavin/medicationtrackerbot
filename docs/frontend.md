...
- **`tab_order` persistence**: the `tab_order` array in `settings_bundle` and the `POST /api/settings/tab-order` endpoint are preserved for a future follow-up (Today card drag-reorder), but no renderer currently consumes the value — Today's card order is hardcoded in `renderToday()`. See `docs/plans/2026-04-XX-today-card-drag-reorder.md`.
