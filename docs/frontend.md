...
3b. `components/wg-icons.js`, `wg-bottom-nav.js`, `wg-sparkline.js`, `wg-phone-chrome.js`, `wg-bp-chart.js`, `wg-weight-chart.js`, `wg-macro-bar.js` — Wandergeek design-system primitives (icon registry, bottom nav, sparkline, phone-chrome, BP chart, single-series weight chart with optional goal overlay, macro bar). Must load before `features/bootstrap.js` mounts the bottom nav, before `today.js` renders sparklines, and before `features/food.js` renders the daily macros card.
...
14. `features/food.js`, `features/bp.js`, `features/weight.js`, `features/meds.js` — extracted feature modules
...
- **Sub-tab groups inside section views stay** (`.med-tabs`, `.workout-tabs`, `.wg-food-subtabs`, `.health-tabs`): use `bindTabGroup()` / `activateTabGroup()`. Health sub-tabs are "Overview" (charts) and "Notes" (diary); Notes loads lazily. Active sub-tab persists to `localStorage` under `mt-<section>-subtab` (e.g. `mt-food-subtab` with values `log` / `meals` / `fooddb`, `mt-meds-subtab` with values `schedule` / `history` / `inventory`); range selectors (`mt-bp-range`, `mt-weight-range`, values `7d` / `30d` / `90d` / `all`, default `30d`) follow the same convention.
