...
3b. `components/wg-icons.js`, `wg-bottom-nav.js`, `wg-sparkline.js`, `wg-phone-chrome.js`, `wg-bp-chart.js`, `wg-weight-chart.js`, `wg-workout-chart.js`, `wg-macro-bar.js` — Wandergeek design-system primitives (icon registry, bottom nav, sparkline, phone-chrome, BP chart, weight chart with optional goal overlay, workout sessions-per-week chart, macro bar). Must load before `features/bootstrap.js` mounts the bottom nav, before `today.js` renders sparklines, before `features/food.js` renders the daily macros card, and before `features/workout.js` renders the Stats sub-tab chart.
...
14. `features/food.js`, `features/bp.js`, `features/weight.js`, `features/meds.js`, `features/workout.js` — extracted feature modules
...
- **Sub-tab groups inside section views stay** (`.med-tabs`, `.workout-tabs`, `.wg-food-subtabs`, `.health-tabs`): use `bindTabGroup()` / `activateTabGroup()`. Health sub-tabs are "Overview" (charts) and "Notes" (diary); Notes loads lazily. Active sub-tab persists to `localStorage` under `mt-<section>-subtab` (e.g. `mt-food-subtab` with values `log` / `meals` / `fooddb`, `mt-meds-subtab` with values `schedule` / `history` / `inventory`, `mt-workouts-subtab` with values `history` / `groups` / `exercises` / `stats`); range selectors (`mt-bp-range`, `mt-weight-range`, `mt-workouts-stats-range`, values `7d` / `30d` / `90d` / `all`, default `30d`) follow the same convention.
- **Food screen shell**: `#food-view.view.wg-screen-stage` mirrors the BP backdrop; the Add Food CTA mounts into a sticky dock (`#food-add-cta-dock`) inside `#food-log-tab`, positioned above the bottom nav via `--wg-bottom-nav-reserved`, and the day navigator + CTA dock are hidden on the `meals` and `fooddb` sub-tabs.
