...
- **Sub-tab groups inside section views stay** (`.med-tabs`, `.workout-tabs`, `.wg-food-subtabs`, `.health-tabs`): use `bindTabGroup()` / `activateTabGroup()`. Health sub-tabs are "Overview" (charts) and "Notes" (diary); Notes loads lazily. Active sub-tab persists to `localStorage` under `mt-<section>-subtab` (e.g. `mt-food-subtab` with values `log` / `meals` / `fooddb`, `mt-meds-subtab` with values `schedule` / `history` / `inventory`, `mt-workouts-subtab` with values `history` / `groups` / `exercises` / `stats`, `mt-health-subtab` with values `overview` / `notes`); range selectors (`mt-bp-range`, `mt-weight-range`, `mt-workouts-stats-range`, `mt-health-range`, values `7d` / `30d` / `90d` / `all`, default `30d` — `mt-health-range` is `7d` / `30d` with default `7d`) follow the same convention.
