...
3b. `components/wg-icons.js`, `wg-bottom-nav.js`, `wg-sparkline.js`, `wg-phone-chrome.js`, `wg-bp-chart.js`, `wg-macro-bar.js` — Wandergeek design-system primitives (icon registry, bottom nav, sparkline, phone-chrome, BP chart, macro bar). Must load before `features/bootstrap.js` mounts the bottom nav, before `today.js` renders sparklines, and before `features/food.js` renders the daily macros card.
...
- **Sub-tab groups inside section views stay** (`.med-tabs`, `.workout-tabs`, `.wg-food-subtabs`, `.health-tabs`): use `bindTabGroup()` / `activateTabGroup()`. Health sub-tabs are "Overview" (charts) and "Notes" (diary); Notes loads lazily. Active sub-tab persists to `localStorage` under `mt-<section>-subtab` (e.g. `mt-food-subtab` with values `log` / `meals` / `fooddb`, `mt-meds-subtab` with values `schedule` / `history` / `inventory`); the BP range selector follows the same `mt-bp-range` convention.
