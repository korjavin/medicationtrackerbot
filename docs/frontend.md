...
**Canonical FAB and modal utilities**: `.wg-fab` (in `styles.css`) is the shared floating-action-button positioner — `position: fixed` at bottom-right, offset above the bottom nav via `--wg-bottom-nav-reserved`. Compose it with `.wg-gloss wg-gloss--sun wg-gloss--lg` for the primary section action (see `#add-bp-btn`). `.wg-modal` + `.wg-modal__title` / `.wg-modal__body` / `.wg-modal__actions` + the field utilities `.wg-field` / `.wg-label` / `.wg-input` / `.wg-select` are the shared modal shell — token-driven, teal-stage-aware, with `.wg-gloss` buttons in the actions row. Future section plans (Food edit, Medication record, Weight, Workouts) must reuse these utilities rather than introduce scoped `.wg-bp-*` / `.wg-food-*` variants; the BP screen is the reference consumer.
